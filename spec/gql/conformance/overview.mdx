...
Every named feature in the conformance catalog has its own page under [`/spec/gql/features/`](/spec/gql/features/match-basic). The full list is in the sidebar.

## 🔌 Beyond GQL

ArcFlow ships 9 documented extensions beyond the standard — live queries, Z-set incremental evaluation, evidence algebra, ASOF joins, GPU GraphBLAS, HNSW vector index, and more. Each has its own page under [`/spec/gql/extensions/`](/spec/gql/extensions/live-queries).

Extensions do not affect ISO conformance — they are opt-in syntax the standard explicitly leaves to implementations.

## 🧪 Test corpus

The full openCypher TCK (3881/3881) plus ArcFlow's regression corpus runs on every engine release. See [TCK Results](/spec/gql/conformance/tck) and [Extension Regressions](/spec/gql/conformance/regressions).
