...
| `10-triggers-and-skills` | `register_skill(NodeCreated{Frame}, proximity_query)` for per-frame reactive analytics + `register_live_proof(NonEmpty)` for sensor-dropout detection |
...
- **Reactive skills, no scheduler.** `register_skill` fires the named
  query on every mutation matching the trigger. The proximity query
  in `10-triggers-and-skills` runs against every new Frame; the
