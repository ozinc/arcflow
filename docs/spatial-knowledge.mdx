...
Recent engine benchmarks show spatial k-nearest throughput around `471K ops/s` on the corrected benchmark surface. The index is dynamic — inserts, updates, and deletes are O(log N) with no rebuild.
