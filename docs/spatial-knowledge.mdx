...
description: "Exact spatial queries backed by a dynamic R*-tree — KNN, radius, bbox, frustum, live geofencing, and graph traversal in one GQL statement"
...
ArcFlow treats spatial coordinates as a first-class query dimension alongside graph relationships. K-nearest, radius, bounding-box, and frustum queries run against the R\*-tree directly. Spatial predicates and graph traversal compose in a single GQL statement — "find the 5 nearest warehouses that have inventory for SKU X" is one query, not two. Position updates trigger live geofence queries within 20ms.
...
In TypeScript, subscribe to geofence enter/exit events via the reactive API:
...
The GPU-uploadable spatial index (`FlatSpatialIndex`) uses flat arrays of pointer-free BVH nodes — `#[repr(C)]` structs uploaded directly to GPU VRAM without transformation. Pointer-based tree structures cannot cross the CPU→GPU boundary.
...
For reactive spatial trigger metrics:
