...
playground:
  ...
  query: |-
    ...
    WITH s, d, r, z, s.reliability * d.confidence AS composite_trust, (r.x - s.x) * (r.x - s.x) + (r.y - s.y) * (r.y - s.y) AS proximity_sq
    RETURN s.name AS sensor, s.type, s.x AS sensor_x, s.y AS sensor_y, s.reliability, d.method, d.confidence, composite_trust, proximity_sq, r.name AS robot, r.x AS robot_x, r.y AS robot_y, r.task, r.battery, z.name AS zone ORDER BY proximity_sq
