...
The unit ArcFlow indexes is the **`SpatialEntry`** — the engine's spatial primitive. When a node is given a position, the engine materializes a `SpatialEntry` for it and stores it in the R\*-tree:

```
SpatialEntry { node_id, coords: [x, y, z], crs }
```

Three fields, ~32 bytes, cache-line friendly: the `node_id` it belongs to, native 3D `coords` (2D points set `z = 0.0`), and the coordinate reference system (`crs`). Every spatial query — K-nearest, radius, bounding-box, frustum — matches over `SpatialEntry` records and returns the **nodes** they point back to, so spatial results compose directly with graph traversal. This is ArcFlow's own term for a positioned thing in the world model; one node has at most one `SpatialEntry` per indexed position.
...
Spatial operation throughput depends on host CPU, memory bandwidth,
and entity count. Measure on the hardware you'll deploy on:
...
| Operation | Behaviour |
|---|---|
| K-nearest, radius search | O(log N) on the R*-tree |
| Frustum / line-crossing checks | Index-narrowed candidate set |
| Zone occupancy | Incremental, edge-triggered |
| Live geofence trigger | Incremental, edge-triggered |
| USD bulk ingest | Bulk packing for large scenes |
...
Beyond point distance, WorldCypher exposes geometry functions over `LineString` and `Polygon` properties — usable inline in `RETURN`/`WHERE`:

```cypher
-- Arc length of a route (LineString)
MATCH (r:Route) RETURN arcflow.length(r.path) AS metres

-- Polygon area; point-in-polygon; geometry intersection
MATCH (z:Zone) RETURN arcflow.area(z.boundary) AS area
MATCH (z:Zone), (a:Agent) WHERE arcflow.contains(z.boundary, a.loc) RETURN a.id
MATCH (r:Route), (z:Zone) WHERE arcflow.intersects(r.path, z.boundary) RETURN r.id, z.name

-- Aggregate convex hull over a set of points
MATCH (a:Agent) RETURN arcflow.convexHull(a.loc) AS hull
```

| Function | Returns |
|---|---|
| `arcflow.length(line)` | Arc length of a `LineString`. |
| `arcflow.area(polygon)` | Area of a `Polygon`. |
| `arcflow.contains(polygon, point)` | `true` if the point lies inside the polygon. |
| `arcflow.intersects(geom_a, geom_b)` | `true` if the two geometries intersect. |
| `arcflow.convexHull(point)` | Aggregate — the convex hull of a `Point` property across matched rows. |

## See Also

- [Spatial Reference](/worldcypher/spatial) — full spatial query syntax, R*-tree, frustum, and raycast
- [Robotics & Perception](/markets/robotics) — sensor fusion pipeline and ROS2 integration patterns
- [Autonomous Systems](/docs/use-cases/autonomous-systems) — multi-agent fleet coordination on a shared spatial world model
- [Temporal Queries](/temporal) — combine spatial position with `AS OF` for historical trajectory queries
- [Digital Twins](/docs/use-cases/digital-twins) — live spatial replica of physical environments
