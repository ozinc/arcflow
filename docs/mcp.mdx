...
`read_query` and `graph_rag` envelopes carry a `snapshot_id` field — the URI of the
snapshot the call observed:
...
Both tools also accept an optional `snapshot_uri` input. When provided, the call is
pinned to that historical snapshot and the response echoes it back. This lets a chat
agent re-run a previous answer against its original world state, or pin a sequence of
related questions to one consistent view.

The HTTP server exposes the same provenance via the `X-Arcflow-Snapshot-Id` and
`X-Arcflow-Manifest-Pin` response headers, and accepts pinning via the `?at=` query
string. See [Snapshot-Pinned Reads](/concepts/snapshots) for the full surface.
...
| Surface | Use when |
|---|---|
| napi-rs in-process | Application code in Node.js |
| `arcflow` CLI binary | Shell-capable agents (Claude Code, Codex, Gemini CLI) |
| MCP server | Cloud chat UIs with no local shell |
...
- [Snapshot-Pinned Reads](/concepts/snapshots) — `snapshot_id` envelope and provenanced answers
- [Skills](/skills) — teach the world model a relationship rule in plain language; compiled once, executed at graph speed forever
