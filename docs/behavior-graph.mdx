...
since: "10.0.0"
status: "preview"
  query: "MERGE (coord:Agent {id: 'coord', name: 'Coordinator', role: 'orchestrator', active: 1}) MERGE (plan:Agent {id: 'planner', name: 'Planner', role: 'planning', active: 1}) MERGE (exec:Agent {id: 'executor', name: 'Executor', role: 'execution', active: 1}) MERGE (guard:Agent {id: 'guardian', name: 'Guardian', role: 'safety', active: 1}) MERGE (s1:Sensor {id: 'cam-1', name: 'Cam-LabA', type: 'camera', reliability: 0.95}) MERGE (s2:Sensor {id: 'lidar-1', name: 'Lidar-LabB', type: 'lidar', reliability: 0.91}) MERGE (coord:Agent {id: 'coord'})-[:DELEGATES_TO {on_event: 'detection', threshold: 0.85, priority: 1}]->(plan:Agent {id: 'planner'}) MERGE (coord:Agent {id: 'coord'})-[:DELEGATES_TO {on_event: 'hazard_alert', threshold: 0.70, priority: 0}]->(guard:Agent {id: 'guardian'}) MERGE (plan:Agent {id: 'planner'})-[:DELEGATES_TO {on_event: 'plan_ready', threshold: 0.80, priority: 2}]->(exec:Agent {id: 'executor'}) MERGE (plan:Agent {id: 'planner'})-[:MONITORS {since_seq: 1, freq_hz: 10}]->(s1:Sensor {id: 'cam-1'}) MERGE (guard:Agent {id: 'guardian'})-[:MONITORS {since_seq: 1, freq_hz: 25}]->(s2:Sensor {id: 'lidar-1'}) MATCH (a:Agent)-[d:DELEGATES_TO]->(b:Agent) RETURN a.name AS from_agent, a.role, d.on_event, d.threshold AS confidence_threshold, d.priority, b.name AS to_agent, b.role ORDER BY d.priority"
playground:
  seed: world
  query: "MERGE (coord:Agent {id: 'coord', name: 'Coordinator', role: 'orchestrator', active: 1})\n\nMERGE (plan:Agent {id: 'planner', name: 'Planner', role: 'planning', active: 1, on_event: 'detection', threshold: 0.85, priority: 1})\n\nMERGE (exec:Agent {id: 'executor', name: 'Executor', role: 'execution', active: 1, on_event: 'plan_ready', threshold: 0.80, priority: 2})\n\nMERGE (guard:Agent {id: 'guardian', name: 'Guardian', role: 'safety', active: 1, on_event: 'hazard_alert', threshold: 0.70, priority: 0})\n\nMERGE (s1:Sensor {id: 'cam-1', name: 'Cam-LabA', type: 'camera', reliability: 0.95, poll_hz: 10})\n\nMERGE (s2:Sensor {id: 'lidar-1', name: 'Lidar-LabB', type: 'lidar', reliability: 0.91, poll_hz: 25})\n\nMERGE (c1:Agent {id: 'coord'})-[:DELEGATES_TO]->(p:Agent {id: 'planner'})\n\nMERGE (c2:Agent {id: 'coord'})-[:DELEGATES_TO]->(g:Agent {id: 'guardian'})\n\nMERGE (p1:Agent {id: 'planner'})-[:DELEGATES_TO]->(e:Agent {id: 'executor'})\n\nMERGE (p2:Agent {id: 'planner'})-[:MONITORS]->(s1:Sensor {id: 'cam-1'})\n\nMERGE (g1:Agent {id: 'guardian'})-[:MONITORS]->(s2:Sensor {id: 'lidar-1'})\n\nMATCH (a:Agent)-[:DELEGATES_TO]->(b:Agent) RETURN a.name AS from_agent, a.role AS from_role, b.name AS to_agent, b.role AS to_role, b.on_event AS trigger, b.threshold AS confidence_threshold, b.priority AS priority ORDER BY b.priority"
