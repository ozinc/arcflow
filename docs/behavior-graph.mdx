...
playground:
  seed: world
  query: |-
    MERGE (coord:Agent {id: 'coord', name: 'Coordinator', role: 'orchestrator', active: 1})
    MERGE (plan:Agent {id: 'planner', name: 'Planner', role: 'planning', active: 1})
    MERGE (exec:Agent {id: 'executor', name: 'Executor', role: 'execution', active: 1})
    MERGE (guard:Agent {id: 'guardian', name: 'Guardian', role: 'safety', active: 1})
    ...
    MATCH (coord:Agent {id: 'coord'}) MATCH (plan:Agent {id: 'planner'}) MERGE (coord)-[:DELEGATES_TO {on_event: 'detection', threshold: 0.85, priority: 1}]->(plan)
    MATCH (coord:Agent {id: 'coord'}) MATCH (guard:Agent {id: 'guardian'}) MERGE (coord)-[:DELEGATES_TO {on_event: 'hazard_alert', threshold: 0.70, priority: 0}]->(guard)
    MATCH (plan:Agent {id: 'planner'}) MATCH (exec:Agent {id: 'executor'}) MERGE (plan)-[:DELEGATES_TO {on_event: 'plan_ready', threshold: 0.80, priority: 2}]->(exec)
    MATCH (plan:Agent {id: 'planner'}) MATCH (s1:Sensor {id: 'cam-1'}) MERGE (plan)-[:MONITORS {since_seq: 1, freq_hz: 10}]->(s1)
    MATCH (guard:Agent {id: 'guardian'}) MATCH (s2:Sensor {id: 'lidar-1'}) MERGE (guard)-[:MONITORS {since_seq: 1, freq_hz: 25}]->(s2)
