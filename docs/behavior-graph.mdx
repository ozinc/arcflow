...
playground:
  ...
  query: |-
    ...
    MERGE (coord:Agent {id: 'coord'})-[:DELEGATES_TO {on_event: 'detection', threshold: 0.85, priority: 1}]->(plan:Agent {id: 'planner'})

    MERGE (coord:Agent {id: 'coord'})-[:DELEGATES_TO {on_event: 'hazard_alert', threshold: 0.70, priority: 0}]->(guard:Agent {id: 'guardian'})

    MERGE (plan:Agent {id: 'planner'})-[:DELEGATES_TO {on_event: 'plan_ready', threshold: 0.80, priority: 2}]->(exec:Agent {id: 'executor'})

    MERGE (plan:Agent {id: 'planner'})-[:MONITORS {since_seq: 1, freq_hz: 10}]->(s1:Sensor {id: 'cam-1'})

    MERGE (guard:Agent {id: 'guardian'})-[:MONITORS {since_seq: 1, freq_hz: 25}]->(s2:Sensor {id: 'lidar-1'})
