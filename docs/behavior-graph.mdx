...
description: "Behavior Trees backed by a knowledge graph — persistent, temporal, queryable"
...
**Behavior trees that persist across restarts, remember their history, and query their own state with Cypher.**

Standard behavior tree engines (UE5, BT.CPP) lose all state when the process exits, limit their blackboard to key-value lookup, and can't answer "what was the agent doing 5 minutes ago?" ArcFlow's Behavior Graph stores tree structure and blackboard state in a WAL-backed knowledge graph. You get `AS OF` temporal queries on any agent, full Cypher over the blackboard, and thousands of concurrent trees sharing a single graph — with zero external infrastructure.
...
| Blackboard | GraphStore (knowledge graph) | Full Cypher queries, not just key-value |
...
| Reactive Sequence | Live queries + subscriptions | Cypher-based reactivity |
...
The blackboard isn't a separate key-value store. It's the **entire graph**. Query it with full Cypher:
...
Evaluate a Cypher query — SUCCESS if rows returned, FAILURE if empty:
...
- **Graph-queryable blackboard** — full Cypher over the knowledge graph, not key-value lookup
...
Your BT Blueprint nodes map to `:BehaviorNode` graph nodes. The Blackboard becomes the full graph. Environment queries become Cypher patterns. The difference: ArcFlow trees persist across restarts, carry temporal history, and scale to thousands of concurrent agents sharing a single knowledge graph. Trade-off: ArcFlow runs outside the game engine process, so you need FFI or MCP integration for tight game-loop coupling.

### C++ Behavior Tree Libraries / ROS2?
Same tick-based evaluation model. ArcFlow adds WAL-durable persistence, Cypher queries as conditions/actions, multi-robot coordination via partitioned workspaces, and temporal sensor fusion across clock domains. Trade-off: dedicated BT libraries have lower per-tick overhead for pure in-memory evaluation; ArcFlow's advantage is queryable persistent state and multi-agent coordination.
...
| Conditions | Cypher queries (row count > 0 = SUCCESS) |
| Actions | Cypher mutations |
| Reactivity | Live queries + subscriptions |
