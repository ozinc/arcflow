...
playground:
  seed: world
  query: |-
    MERGE (dir:Agent {id: 'director', name: 'Director', role: 'orchestrator', active: true})
    MERGE (per:Agent {id: 'perceiver', name: 'Perceiver', role: 'perception', active: true})
    MERGE (pln:Agent {id: 'planner', name: 'Planner', role: 'planning', active: true})
    ...
    MATCH (dir:Agent {id: 'director'}), (per:Agent {id: 'perceiver'}) MERGE (dir)-[:DELEGATES_TO {task: 'monitor', confidence_floor: 0.85, seq: 1}]->(per)
    MATCH (dir:Agent {id: 'director'}), (pln:Agent {id: 'planner'}) MERGE (dir)-[:DELEGATES_TO {task: 'plan', confidence_floor: 0.90, seq: 2}]->(pln)
    MATCH (per:Agent {id: 'perceiver'}), (s1:Sensor {id: 'cam-1'}) MERGE (per)-[:MONITORS {since_seq: 1, priority: 1}]->(s1)
    MATCH (per:Agent {id: 'perceiver'}), (s2:Sensor {id: 'lidar-1'}) MERGE (per)-[:MONITORS {since_seq: 2, priority: 2}]->(s2)
    MATCH (ag:Agent)-[m:MONITORS]->(s:Sensor) RETURN ag.name AS agent, ag.role, m.since_seq, m.priority, s.name AS sensor, s.type, s.reliability ORDER BY m.priority
