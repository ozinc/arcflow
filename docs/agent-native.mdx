...
status: "stable"
playground:
  query: "MERGE (auth:File {id: 'auth', name: 'auth.ts', filepath: 'src/auth.ts', lines: 320, layer: 'data'}) MERGE (db:File {id: 'db', name: 'db.ts', filepath: 'src/db.ts', lines: 240, layer: 'data'}) MERGE (api:File {id: 'api', name: 'routes.ts', filepath: 'src/api/routes.ts', lines: 155, layer: 'api'}) MERGE (main:File {id: 'main', name: 'main.ts', filepath: 'src/main.ts', lines: 85, layer: 'app'}) MATCH (f:File) RETURN f.name AS file, f.filepath, f.lines, f.layer ORDER BY f.lines DESC"
---
...
With ArcFlow: a spatial-temporal, confidence-scored world model. Graph traversal, vector search, 29 algorithms — all of it accessible as files. The agent writes a `.cypher` query. The CLI executes it. The result appears as `.json`. The agent reads it. **No protocol. No tokens. No round-trip. The filesystem is the API.**
...
| Consumer | Surface | Characteristic | Why |
|---|---|---|---|
| **Application daemons, services** | napi-rs in-process | **Function call** | Same memory, zero boundary |
| **Claude Code, Codex, Cursor, shell scripts** | `arcflow` CLI binary | **Shell exec** | Shell-native, composable, linked Rust binary |
| **Claude.ai, ChatGPT, Gemini web** | MCP server | **Protocol RPC** | No local shell — protocol is the only bridge |
| **Python / shell pipelines** | `arcflow` CLI binary | **Shell exec** | Same as CLI agents |
...
### Project the World Model — the agent-grep workflow

For agents that speak Unix more fluently than Cypher, project the entire snapshot to a
...
The output is a deterministic on-disk layout: `nodes/<Label>/<id>.json`,
`edges/<Type>/<id>.json`, plus index files. The agent can now `rg`, `grep`, `cat`, and
`jq` the world model with no protocol layer at all:

```bash
# Find low-battery observed robots — pure ripgrep
...
projections is a true graph-state diff. See [Filesystem Projection](/cli/project).
...
|---|---|---|
| **Latency** | Disk I/O | JSON-RPC round-trip |
| **Token cost** | Zero | Per-call protocol overhead |
...
- [ArcFlow for Coding Agents](/docs/guides/agent-quickstart) — CLI patterns, structured errors, batch execution, and checkpointing
- [Filesystem Projection](/cli/project) — `arcflow project` writes the world model as a directory tree
- [Snapshot-Pinned Reads](/concepts/snapshots) — citeable snapshot URIs for provenanced answers
...
- [Skills](/skills) — teach the world model a new relationship rule in plain language; compiled once, executed at graph speed forever
