...
playground:
  query: |-
    MERGE (auth:File {id: 'auth', name: 'auth.ts', path: 'src/auth.ts', lines: 320, layer: 'data'})
    MERGE (db:File {id: 'db', name: 'db.ts', path: 'src/db.ts', lines: 240, layer: 'data'})
    MERGE (api:File {id: 'api', name: 'routes.ts', path: 'src/api/routes.ts', lines: 155, layer: 'api'})
    MERGE (main:File {id: 'main', name: 'main.ts', path: 'src/main.ts', lines: 85, layer: 'app'})
    MATCH (auth:File {id: 'auth'}), (db:File {id: 'db'}) MERGE (auth)-[:IMPORTS {seq: 1}]->(db)
    MATCH (main:File {id: 'main'}), (auth:File {id: 'auth'}) MERGE (main)-[:IMPORTS {seq: 2}]->(auth)
    MATCH (api:File {id: 'api'}), (auth:File {id: 'auth'}) MERGE (api)-[:IMPORTS {seq: 3}]->(auth)
    MATCH (main:File {id: 'main'}), (db:File {id: 'db'}) MERGE (main)-[:IMPORTS {seq: 4}]->(db)
    MATCH (f:File)-[i:IMPORTS]->(dep:File) RETURN f.name AS file, f.path, f.lines, f.layer, dep.name AS imports, dep.layer AS dep_layer, i.seq ORDER BY f.lines DESC
