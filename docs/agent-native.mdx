...
playground:
  query: |-
    MERGE (auth:Module {id: 'auth', name: 'auth.ts', path: 'src/auth.ts', loc: 320, critical: true})
    MERGE (db:Module {id: 'db', name: 'db.ts', path: 'src/db/client.ts', loc: 180, critical: true})
    MERGE (api:Module {id: 'api', name: 'routes.ts', path: 'src/api/routes.ts', loc: 155, critical: false})
    MERGE (utils:Module {id: 'utils', name: 'logger.ts', path: 'src/utils/logger.ts', loc: 60, critical: false})
    MATCH (auth:Module {id: 'auth'}), (db:Module {id: 'db'}) MERGE (auth)-[:IMPORTS {static: true, critical: true}]->(db)
    MATCH (api:Module {id: 'api'}), (auth:Module {id: 'auth'}) MERGE (api)-[:IMPORTS {static: true, critical: true}]->(auth)
    MATCH (api:Module {id: 'api'}), (db:Module {id: 'db'}) MERGE (api)-[:IMPORTS {static: true, critical: true}]->(db)
    MATCH (auth:Module {id: 'auth'}), (utils:Module {id: 'utils'}) MERGE (auth)-[:IMPORTS {static: false, critical: false}]->(utils)
    MATCH (m:Module)-[i:IMPORTS]->(dep:Module) RETURN m.name AS file, m.path, m.loc AS lines, i.critical AS critical_dep, dep.name AS depends_on, dep.critical AS dep_critical ORDER BY m.loc DESC
