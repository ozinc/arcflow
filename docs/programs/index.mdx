...
This is a complete deployment unit. The engine validates the GPU, registers the skills and triggers, and records the output schema — all before any data arrives.

## What the manifest layer does today

The declaration and registry layer is live: `CREATE PROGRAM` / `DROP PROGRAM` parse every clause into a durable manifest (WAL-journaled, replayed on recovery), `REQUIRES GPU` enforces the SM compute-capability gate at install time, and the registry procedures (`arcflow.programs.list`, `describe`, `validate`, `find_by_capability`) read it back.

The **executor dispatch layer is target-state**: the manifest records the `EXECUTOR` endpoint, heartbeat interval, and VRAM floor, but the engine does not yet open the executor connection, send heartbeats, or ingest executor results. `arcflow.programs.health` and the VRAM portion of `REQUIRES GPU` are surfaced for the manifest contract but are not yet enforced. Treat a Program as a validated, queryable deployment contract — not yet a running pipeline.
...
- [Skills](/skills) — `CREATE SKILL` for compiled graph-pattern logic, introspection, and provenance
- [Plugin Management](/cli/plugin) — extend the runtime with external inference backends
- [GPU Acceleration](/gpu) — compute-capability guards and CUDA kernel dispatch
- [Use Case: Sports Analytics](/docs/use-cases/sports-analytics) — Programs in a real-time broadcast pipeline
- [Use Case: Autonomous Systems](/docs/use-cases/autonomous-systems) — Programs for sensor fusion and perception
