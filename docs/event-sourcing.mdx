...
description: "Every state change is a temporal fact — replay any moment, no external broker needed"
...
ArcFlow's WAL automatically records every mutation as a temporal fact. You get point-in-time `AS OF` queries, deterministic replay, change data capture, and Cypher over the full event history — no event bus, no projection rebuilds, no separate read model. One binary, one process, four infrastructure systems eliminated.
...
| Event Store | `db.mutations()` + CDC log | Queryable with Cypher, not just sequential replay |
| Aggregate | Graph subgraph | Relationships between entities, not isolated aggregates |
| Projection | `MATCH ... RETURN` | Real-time Cypher queries, no projection rebuild |
...
| CQRS Read Model | Live queries + subscriptions | Reactive, auto-updating read models |
...
MATCH (o:Order) AS OF 1711324800000
...
```cypher
MATCH (a:Account {id: 'ACC-789'}) AS OF 1710511200000
...
Closest architectural match. ArcFlow adds graph-native queries (Cypher vs Datalog), GPU acceleration (154M PageRank nodes/sec), vector search, and spatial primitives. Trade-off: immutable databases support distributed clusters and have battle-tested production deployments at scale; ArcFlow is single-process with higher single-machine throughput.
