...
playground:
  ...
  query: |-
    ...
    WITH s, d, r, occ, z, s.reliability * d.confidence AS observation_quality
    RETURN occ.entered_seq AS event_seq, 'entity_observed' AS event_type, r.name AS entity, r.task, r.x AS pos_x, r.y AS pos_y, r.speed AS velocity, r.battery AS energy, observation_quality, d.confidence AS detection_confidence, d.method AS detection_method, s.name AS reporting_sensor, z.name AS zone ORDER BY event_seq
...
-- State 1 hour ago (no projection needed — just query the past)
...
-- State 1 hour ago
MATCH (o:Order {id: 'ORD-001'}) AS OF (timestamp() - 3600000)
RETURN o.status
-- Result: "confirmed"
...
```cypher
-- Snapshot state at daily intervals
MATCH (o:Order) AS OF (timestamp() - 86400000)  -- yesterday
...
MATCH (p:Product {sku: 'WP-100'}) AS OF (timestamp() - 60000)
