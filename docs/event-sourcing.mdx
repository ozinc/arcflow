...
Closest architectural match. ArcFlow adds graph-native queries (GQL vs Datalog), GPU acceleration, vector search, and spatial primitives. Trade-off: immutable databases support distributed clusters and have battle-tested production deployments at scale; ArcFlow is single-process with higher single-machine throughput.
