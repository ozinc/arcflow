...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS hazard, count(r) AS entities, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, hazard, entities, avg_battery, composite_trust, CASE WHEN composite_trust >= 0.90 THEN 'nominal' WHEN composite_trust >= 0.75 THEN 'partial' ELSE 'stale' END AS status ORDER BY composite_trust DESC"
---

# Live Queries

*Canonical surface for the [Live Surface](/docs/concepts/layers/live-surface) layer. Part of the LIVE family in the four-keyword vocabulary — see [Execution Models](/docs/concepts/execution-models) for how LIVE / TRIGGER / SKILL / PROGRAM compose.*
...
> **Information-theoretic view:** delivering only the delta is *residual coding* — the subscriber already holds the prior result, so the engine sends only the **surprise** (what changed), never the whole result set. Same principle as WAL sync; the engine can also measure it directly — see [the Information Layer](/docs/concepts/information-layer).
...
| | LIVE | TRIGGER |
|---|---|---|
| **Syntax** | `CREATE LIVE VIEW`, `LIVE MATCH`, `LIVE CALL` | `CREATE TRIGGER` |
| **Semantics** | Always current — maintained continuously as the graph mutates | Fire-once — executes exactly once per matching graph event |
| **State** | Z-set maintained — result set is always current | Stateless — no maintained result |
| **Use when** | You need a result set that is always true | You need a one-time side effect per event |
...
See [Triggers](/worldcypher/triggers) for the full `CREATE TRIGGER` reference.

---

## See also

- [Triggers](/worldcypher/triggers) — `CREATE TRIGGER` for fire-once event bindings
