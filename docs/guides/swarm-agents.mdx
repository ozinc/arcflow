...
playground:
  ...
  query: |-
    ...
    MERGE (l:Agent {id: 'swarm-lead'})-[:COORDINATES {intent: 'patrol', priority: 1, confidence_floor: 0.88}]->(f1:Agent {id: 'swarm-f1'})

    MERGE (l:Agent {id: 'swarm-lead'})-[:COORDINATES {intent: 'inspect', priority: 2, confidence_floor: 0.85}]->(f2:Agent {id: 'swarm-f2'})

    MERGE (l:Agent {id: 'swarm-lead'})-[:COORDINATES {intent: 'hold', priority: 3, confidence_floor: 0.80}]->(f3:Agent {id: 'swarm-f3'})

    MERGE (f1:Agent {id: 'swarm-f1'})-[:MONITORS {freq_hz: 25}]->(s1:Sensor {id: 'cam-1'})

    MERGE (f2:Agent {id: 'swarm-f2'})-[:MONITORS {freq_hz: 10}]->(s2:Sensor {id: 'lidar-1'})
