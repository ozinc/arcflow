...
description: "A shared world model as the coordination substrate for AI agent swarms — persistent shared state, reactive task assignment, temporal audit, and confidence-weighted knowledge."
section: "guides"
status: "stable"
---
...
That is what ArcFlow provides as the coordination substrate. The world model is the swarm's shared mind. Agents register, write observations, claim tasks, and query the combined knowledge of every other agent — with full temporal history, confidence scoring, and reactive notification when anything changes.
...
## Reactive coordination — subscribe, not poll
...
ArcFlow's reactive layer inverts this:
...
| Task queue | External broker (Redis, RabbitMQ) | `:Task` nodes with status edges — queryable, auditable |
| Reactive assignment | Polling | `db.subscribe()` — fires on task creation, zero latency |
