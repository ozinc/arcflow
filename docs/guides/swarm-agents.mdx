...
playground:
  ...
  query: |-
    ...
    MATCH (l:Agent {id: 'swarm-lead'}) MATCH (f1:Agent {id: 'swarm-f1'}) MERGE (l)-[:COORDINATES {intent: 'patrol', priority: 1, confidence_floor: 0.88}]->(f1)
    MATCH (l:Agent {id: 'swarm-lead'}) MATCH (f2:Agent {id: 'swarm-f2'}) MERGE (l)-[:COORDINATES {intent: 'inspect', priority: 2, confidence_floor: 0.85}]->(f2)
    MATCH (l:Agent {id: 'swarm-lead'}) MATCH (f3:Agent {id: 'swarm-f3'}) MERGE (l)-[:COORDINATES {intent: 'hold', priority: 3, confidence_floor: 0.80}]->(f3)
    MATCH (f1:Agent {id: 'swarm-f1'}) MATCH (s1:Sensor {id: 'cam-1'}) MERGE (f1)-[:MONITORS {freq_hz: 25}]->(s1)
    MATCH (f2:Agent {id: 'swarm-f2'}) MATCH (s2:Sensor {id: 'lidar-1'}) MERGE (f2)-[:MONITORS {freq_hz: 10}]->(s2)
