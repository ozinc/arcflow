...
playground:
  seed: world
  query: "MERGE (leader:Agent {id: 'swarm-lead', name: 'Alpha-Lead', role: 'leader', active: 1, x: 0.0, y: 0.0}) MERGE (f1:Agent {id: 'swarm-f1', name: 'Follower-01', role: 'follower', active: 1, x: 18.0, y: 8.5}) MERGE (f2:Agent {id: 'swarm-f2', name: 'Follower-02', role: 'follower', active: 1, x: -17.0, y: -9.0}) MERGE (f3:Agent {id: 'swarm-f3', name: 'Follower-03', role: 'follower', active: 1, x: 2.0, y: 1.5}) MERGE (s1:Sensor {id: 'cam-1', name: 'Cam-LabA', type: 'camera', reliability: 0.95}) MERGE (s2:Sensor {id: 'lidar-1', name: 'Lidar-LabB', type: 'lidar', reliability: 0.91}) MERGE (l:Agent {id: 'swarm-lead'})-[:COORDINATES {intent: 'patrol', priority: 1, confidence_floor: 0.88}]->(f1:Agent {id: 'swarm-f1'}) MERGE (l:Agent {id: 'swarm-lead'})-[:COORDINATES {intent: 'inspect', priority: 2, confidence_floor: 0.85}]->(f2:Agent {id: 'swarm-f2'}) MERGE (l:Agent {id: 'swarm-lead'})-[:COORDINATES {intent: 'hold', priority: 3, confidence_floor: 0.80}]->(f3:Agent {id: 'swarm-f3'}) MERGE (f1:Agent {id: 'swarm-f1'})-[:MONITORS {freq_hz: 25}]->(s1:Sensor {id: 'cam-1'}) MERGE (f2:Agent {id: 'swarm-f2'})-[:MONITORS {freq_hz: 10}]->(s2:Sensor {id: 'lidar-1'}) MATCH (leader:Agent)-[cmd:COORDINATES]->(follower:Agent) RETURN leader.name AS swarm_leader, follower.name AS agent, follower.x AS agent_x, follower.y AS agent_y, cmd.intent AS assigned_intent, cmd.priority AS order, cmd.confidence_floor AS min_trust ORDER BY cmd.priority"
