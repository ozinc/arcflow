...
playground:
  query: "MERGE (f1:File {id: 'src/main.ts', name: 'main.ts', type: 'entrypoint', loc: 85, critical: 1})\n\nMERGE (f2:File {id: 'src/lib/db.ts', name: 'db.ts', type: 'library', loc: 240, critical: 1})\n\nMERGE (f3:File {id: 'src/lib/auth.ts', name: 'auth.ts', type: 'library', loc: 320, critical: 1})\n\nMERGE (f4:File {id: 'src/api/routes.ts', name: 'routes.ts', type: 'api', loc: 155, critical: 0})\n\nMERGE (f5:File {id: 'src/utils/logger.ts', name: 'logger.ts', type: 'utility', loc: 60, critical: 0})\n\nMERGE (f1a:File {id: 'src/main.ts'})-[:IMPORTS]->(f2a:File {id: 'src/lib/db.ts'})\n\nMERGE (f1b:File {id: 'src/main.ts'})-[:IMPORTS]->(f3a:File {id: 'src/lib/auth.ts'})\n\nMERGE (f4a:File {id: 'src/api/routes.ts'})-[:IMPORTS]->(f2b:File {id: 'src/lib/db.ts'})\n\nMERGE (f3b:File {id: 'src/lib/auth.ts'})-[:IMPORTS]->(f5a:File {id: 'src/utils/logger.ts'})\n\nMATCH (f:File)-[:IMPORTS]->(dep:File) WITH dep.name AS dependency, dep.type AS dep_type, dep.loc AS dep_loc, dep.critical AS critical, count(f) AS dependents RETURN dependency, dep_type, dep_loc, critical, dependents ORDER BY dependents DESC"
