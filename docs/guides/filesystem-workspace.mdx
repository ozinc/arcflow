...
playground:
  query: |-
    MERGE (f1:File {id: 'src/main.ts', name: 'main.ts', type: 'entrypoint', loc: 85, critical: 1})
    ...
    MATCH (f1:File {id: 'src/main.ts'}) MATCH (f2:File {id: 'src/lib/db.ts'}) MERGE (f1)-[:IMPORTS {static: 1}]->(f2)
    MATCH (f1:File {id: 'src/main.ts'}) MATCH (f3:File {id: 'src/lib/auth.ts'}) MERGE (f1)-[:IMPORTS {static: 1}]->(f3)
    MATCH (f4:File {id: 'src/api/routes.ts'}) MATCH (f2:File {id: 'src/lib/db.ts'}) MERGE (f4)-[:IMPORTS {static: 1}]->(f2)
    MATCH (f3:File {id: 'src/lib/auth.ts'}) MATCH (f5:File {id: 'src/utils/logger.ts'}) MERGE (f3)-[:IMPORTS {static: 0}]->(f5)
