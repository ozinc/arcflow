...
playground:
  query: |-
    ...
    MERGE (f1:File {id: 'src/main.ts'})-[:IMPORTS {static: 1}]->(f2:File {id: 'src/lib/db.ts'})

    MERGE (f1:File {id: 'src/main.ts'})-[:IMPORTS {static: 1}]->(f3:File {id: 'src/lib/auth.ts'})

    MERGE (f4:File {id: 'src/api/routes.ts'})-[:IMPORTS {static: 1}]->(f2:File {id: 'src/lib/db.ts'})

    MERGE (f3:File {id: 'src/lib/auth.ts'})-[:IMPORTS {static: 0}]->(f5:File {id: 'src/utils/logger.ts'})
