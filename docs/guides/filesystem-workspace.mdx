...
playground:
  query: |-
    MERGE (f1:File {id: 'src/main.ts', name: 'main.ts', type: 'entrypoint', loc: 85, critical: true})
    MERGE (f2:File {id: 'src/lib/db.ts', name: 'db.ts', type: 'library', loc: 240, critical: true})
    MERGE (f3:File {id: 'src/lib/auth.ts', name: 'auth.ts', type: 'library', loc: 320, critical: true})
    MERGE (f4:File {id: 'src/api/routes.ts', name: 'routes.ts', type: 'api', loc: 155, critical: false})
    MERGE (f5:File {id: 'src/utils/logger.ts', name: 'logger.ts', type: 'utility', loc: 60, critical: false})
    MATCH (f1:File {id: 'src/main.ts'}), (f2:File {id: 'src/lib/db.ts'}) MERGE (f1)-[:IMPORTS {static: true}]->(f2)
    MATCH (f1:File {id: 'src/main.ts'}), (f3:File {id: 'src/lib/auth.ts'}) MERGE (f1)-[:IMPORTS {static: true}]->(f3)
    MATCH (f4:File {id: 'src/api/routes.ts'}), (f2:File {id: 'src/lib/db.ts'}) MERGE (f4)-[:IMPORTS {static: true}]->(f2)
    MATCH (f3:File {id: 'src/lib/auth.ts'}), (f5:File {id: 'src/utils/logger.ts'}) MERGE (f3)-[:IMPORTS {static: false}]->(f5)
    MATCH (f:File)-[:IMPORTS]->(dep:File) WITH dep.name AS dependency, dep.type, dep.loc, dep.critical, count(f) AS dependents RETURN dependency, type, loc, critical, dependents ORDER BY dependents DESC
