...
description: "Connect ArcFlow with any PostgreSQL client using familiar SQL tooling. Understand how the PostgreSQL wire protocol works as a bridge into GQL (ISO/IEC 39075) and WorldCypher."
section: "guides"
order: 10
since: "4.0.0"
status: "stable"
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS zone_risk, count(r) AS robots, avg(r.battery) AS avg_energy, avg(s.reliability * d.confidence) AS avg_composite_trust RETURN zone, zone_risk, robots, avg_energy, avg_composite_trust, CASE WHEN avg_composite_trust >= 0.9 THEN 'trusted' WHEN avg_composite_trust >= 0.75 THEN 'acceptable' ELSE 'gaps - investigate' END AS observability ORDER BY avg_composite_trust DESC"
playground:
  seed: world
...
Graph databases spent two decades without that. Every vendor shipped a different query language: Neo4j had Cypher, Apache TinkerPop had Gremlin, TigerGraph had GSQL, ArangoDB had AQL. Each powerful. None interoperable. You learned one, and your knowledge didn't transfer.
