---
title: "Lakehouse Fast-Path"
kind: guide
description: "Ingest high-cardinality immutable rows (frames, telemetry, event-stream rows) without materialising them into engine RAM. The Lakehouse fast-path keeps observation rows in Hive-partitioned Parquet, accessed via virtual labels; the graph holds typed schema + adjacency + a catalog pointer."
status: "stable"
---

# Lakehouse Fast-Path

The **Lakehouse fast-path** ingests high-cardinality immutable rows — frames, telemetry samples, event-stream rows — without materialising them into engine RAM. Such rows live in Hive-partitioned Parquet and are read in place through [virtual labels](/docs/worldcypher/statements/create-node-label); the graph itself holds only typed schema, adjacency, and a catalog pointer.
...
2. **Is the class an immutable observation row?** Yes → make it a Virtual label.
...
The full [Virtual Labels Over Parquet cookbook](https://github.com/ozinc/arcflow/tree/main/examples/virtual-labels-over-parquet) walks through a runnable example end-to-end.
...
- [Virtual Labels Over Parquet](https://github.com/ozinc/arcflow/tree/main/examples/virtual-labels-over-parquet) — the runnable cookbook recipe.
...
- [CREATE NODE LABEL](/docs/worldcypher/statements/create-node-label) — the `VIRTUAL FROM PARTITION` DDL reference.
