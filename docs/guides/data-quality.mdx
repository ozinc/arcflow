...
description: "Automated DQ, continuous expectation testing, drill-through reconciliation, and auto-generated data lineage — the ML-grade data infrastructure that BI always needed"
section: "guides"
order: 8
since: "4.0.0"
status: "stable"
...
BI teams have lived with a fundamental gap for decades: ML infrastructure had Great Expectations, dbt tests, feature stores with lineage tracking, and expectation frameworks with drill-through. BI had pivot tables, manual spot checks, and Excel. ArcFlow closes that gap. Live views, confidence scoring, temporal snapshots, and graph-native lineage tracking give BI the same data quality infrastructure that ML teams have had — with zero infrastructure overhead, embedded in the same database that serves your queries.
...
MATCH (row) FROM VIEW revenue_by_region AS OF 1710511200000
  RETURN row.region, row.revenue
...
- [Reactive / Live Delta Stack](/docs/reactive) — Z-set algebra internals and live view mechanics
- [Event Sourcing](/docs/event-sourcing) — WAL, AS OF queries, deterministic replay
- [Algorithms](/docs/algorithms) — `algo.factContradiction()`, `algo.entityFreshness()`, `algo.staleEmbeddings()`
- [Trusted RAG](/docs/trusted-rag) — Confidence scoring and provenance trails
- [Constructors](/docs/constructors) — PROCESS NODE, REPROCESS EDGES pipeline patterns
