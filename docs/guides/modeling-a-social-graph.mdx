...
playground:
  ...
  query: |-
    ...
    MERGE (c:Agent {id: 'coord-01'})-[:COMMANDS {channel: 'primary', latency_ms: 12, trust: 0.97}]->(s1:Agent {id: 'scout-01'})

    MERGE (c:Agent {id: 'coord-01'})-[:COMMANDS {channel: 'primary', latency_ms: 14, trust: 0.94}]->(s2:Agent {id: 'scout-02'})

    MERGE (c:Agent {id: 'coord-01'})-[:COMMANDS {channel: 'secure', latency_ms: 8, trust: 0.99}]->(g:Agent {id: 'guard-01'})

    MERGE (s1:Agent {id: 'scout-01'})-[:COVERS {assigned_seq: 1}]->(z1:Zone {id: 'lab-a'})

    MERGE (s2:Agent {id: 'scout-02'})-[:COVERS {assigned_seq: 1}]->(z2:Zone {id: 'lab-b'})

    MERGE (g:Agent {id: 'guard-01'})-[:COVERS {assigned_seq: 1}]->(z3:Zone {id: 'corridor'})
