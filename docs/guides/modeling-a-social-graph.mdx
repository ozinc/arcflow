...
status: "stable"
playground:
  query: "MERGE (coord:Agent {id: 'coord-01', name: 'Coordinator', role: 'coordinator', level: 1, active: 1}) MERGE (scout1:Agent {id: 'scout-01', name: 'Scout-Alpha', role: 'scout', level: 2, active: 1, channel: 'primary', trust: 0.97, latency_ms: 12}) MERGE (scout2:Agent {id: 'scout-02', name: 'Scout-Beta', role: 'scout', level: 2, active: 1, channel: 'primary', trust: 0.94, latency_ms: 14}) MERGE (guard:Agent {id: 'guard-01', name: 'Guard-Prime', role: 'guard', level: 2, active: 1, channel: 'secure', trust: 0.99, latency_ms: 8}) MERGE (z1:Zone {id: 'lab-a', name: 'Lab-A', hazard: 0.2}) MERGE (z2:Zone {id: 'lab-b', name: 'Lab-B', hazard: 0.3}) MERGE (z3:Zone {id: 'corridor', name: 'Corridor', hazard: 0.5}) MERGE (c1:Agent {id: 'coord-01'})-[:COMMANDS]->(s1:Agent {id: 'scout-01'}) MERGE (c2:Agent {id: 'coord-01'})-[:COMMANDS]->(s2:Agent {id: 'scout-02'}) MERGE (c3:Agent {id: 'coord-01'})-[:COMMANDS]->(g:Agent {id: 'guard-01'}) MERGE (s1a:Agent {id: 'scout-01'})-[:COVERS]->(z1a:Zone {id: 'lab-a'}) MERGE (s2a:Agent {id: 'scout-02'})-[:COVERS]->(z2a:Zone {id: 'lab-b'}) MERGE (g1:Agent {id: 'guard-01'})-[:COVERS]->(z3a:Zone {id: 'corridor'}) MATCH (commander:Agent)-[:COMMANDS]->(agent:Agent)-[:COVERS]->(zone:Zone) RETURN commander.name AS commander, agent.name AS agent, agent.role AS role, agent.channel AS channel, agent.trust AS command_trust, agent.latency_ms AS latency_ms, zone.name AS coverage_zone, zone.hazard AS zone_risk ORDER BY agent.trust DESC"
