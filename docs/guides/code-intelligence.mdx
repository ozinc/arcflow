...
status: "stable"
playground:
  query: "MERGE (m1:Module {id: 'mod-auth', name: 'auth', filepath: 'src/auth.ts', loc: 320, coverage: 0.91}) MERGE (m2:Module {id: 'mod-db', name: 'database', filepath: 'src/db/client.ts', loc: 180, coverage: 0.73}) MERGE (f1:Func {id: 'fn-login', name: 'login', module: 'auth', line: 42, complexity: 4, tested: 1, exported: 1, hot_path: 1}) MERGE (f2:Func {id: 'fn-validate', name: 'validateToken', module: 'auth', line: 28, complexity: 2, tested: 1, exported: 0, hot_path: 1}) MERGE (f3:Func {id: 'fn-query', name: 'executeQuery', module: 'database', line: 15, complexity: 3, tested: 0, exported: 1, hot_path: 0}) MERGE (m1:Module {id: 'mod-auth'})-[:CONTAINS]->(f1:Func {id: 'fn-login'}) MERGE (m1a:Module {id: 'mod-auth'})-[:CONTAINS]->(f2:Func {id: 'fn-validate'}) MERGE (m2a:Module {id: 'mod-db'})-[:CONTAINS]->(f3:Func {id: 'fn-query'}) MERGE (f1a:Func {id: 'fn-login'})-[:CALLS]->(f2a:Func {id: 'fn-validate'}) MERGE (f2b:Func {id: 'fn-validate'})-[:CALLS]->(f3a:Func {id: 'fn-query'}) MATCH (m:Module)-[:CONTAINS]->(caller:Func)-[:CALLS]->(dep:Func) RETURN m.name AS module, m.coverage AS test_coverage, caller.name AS function, caller.complexity AS complexity, caller.exported AS exported, caller.hot_path AS hot_path, dep.name AS calls, dep.tested AS dep_tested ORDER BY caller.complexity DESC"
...
- [ArcFlow for Coding Agents](/docs/guides/agent-quickstart) — structured errors, batch execution, and impact analysis
- [Graph Algorithms](/algorithms) — PageRank and community detection for codebase importance ranking
- [Use Case: Agent Tooling](/docs/use-cases/agent-tooling) — ad-hoc graph processing for coding agents
