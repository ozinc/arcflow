...
playground:
  query: |-
    MERGE (m1:Module {id: 'mod-auth', name: 'auth', filepath: 'src/auth.ts', loc: 320, coverage: 0.91})
    ...
    MATCH (m1:Module {id: 'mod-auth'}) MATCH (f1:Function {id: 'fn-login'}) MERGE (m1)-[:CONTAINS {exported: 1}]->(f1)
    MATCH (m1:Module {id: 'mod-auth'}) MATCH (f2:Function {id: 'fn-validate'}) MERGE (m1)-[:CONTAINS {exported: 0}]->(f2)
    MATCH (m2:Module {id: 'mod-db'}) MATCH (f3:Function {id: 'fn-query'}) MERGE (m2)-[:CONTAINS {exported: 1}]->(f3)
    MATCH (f1:Function {id: 'fn-login'}) MATCH (f2:Function {id: 'fn-validate'}) MERGE (f1)-[:CALLS {hot_path: 1, call_site: 42}]->(f2)
    MATCH (f2:Function {id: 'fn-validate'}) MATCH (f3:Function {id: 'fn-query'}) MERGE (f2)-[:CALLS {hot_path: 0, call_site: 31}]->(f3)
