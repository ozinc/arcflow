...
playground:
  query: |-
    ...
    MERGE (f1:Function {id: 'fn-login', name: 'login', module: 'auth', line: 42, complexity: 4, tested: true})
    MERGE (f2:Function {id: 'fn-validate', name: 'validateToken', module: 'auth', line: 28, complexity: 2, tested: true})
    MERGE (f3:Function {id: 'fn-query', name: 'executeQuery', module: 'database', line: 15, complexity: 3, tested: false})
    MATCH (m1:Module {id: 'mod-auth'}), (f1:Function {id: 'fn-login'}) MERGE (m1)-[:CONTAINS {exported: true}]->(f1)
    MATCH (m1:Module {id: 'mod-auth'}), (f2:Function {id: 'fn-validate'}) MERGE (m1)-[:CONTAINS {exported: false}]->(f2)
    MATCH (m2:Module {id: 'mod-db'}), (f3:Function {id: 'fn-query'}) MERGE (m2)-[:CONTAINS {exported: true}]->(f3)
    MATCH (f1:Function {id: 'fn-login'}), (f2:Function {id: 'fn-validate'}) MERGE (f1)-[:CALLS {hot_path: true, call_site: 42}]->(f2)
    MATCH (f2:Function {id: 'fn-validate'}), (f3:Function {id: 'fn-query'}) MERGE (f2)-[:CALLS {hot_path: false, call_site: 31}]->(f3)
