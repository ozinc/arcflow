...
playground:
  query: |-
    ...
    MERGE (m1:Module {id: 'mod-auth'})-[:CONTAINS {exported: 1}]->(f1:Function {id: 'fn-login'})

    MERGE (m1:Module {id: 'mod-auth'})-[:CONTAINS {exported: 0}]->(f2:Function {id: 'fn-validate'})

    MERGE (m2:Module {id: 'mod-db'})-[:CONTAINS {exported: 1}]->(f3:Function {id: 'fn-query'})

    MERGE (f1:Function {id: 'fn-login'})-[:CALLS {hot_path: 1, call_site: 42}]->(f2:Function {id: 'fn-validate'})

    MERGE (f2:Function {id: 'fn-validate'})-[:CALLS {hot_path: 0, call_site: 31}]->(f3:Function {id: 'fn-query'})
