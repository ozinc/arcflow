...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WHERE s.reliability >= 0.9 AND d.confidence >= 0.88 WITH s, d, r, z, s.reliability * d.confidence AS composite_trust RETURN composite_trust, s.name AS evidence_source, s.type AS modality, d.method AS observation_method, r.name AS fact_subject, r.task, r.x AS subject_x, r.y AS subject_y, z.name AS spatial_context, z.hazard AS context_risk, CASE WHEN composite_trust >= 0.92 THEN 'tier-1 - inject verbatim' WHEN composite_trust >= 0.85 THEN 'tier-2 - inject with caveat' ELSE 'tier-3 - summarize only' END AS retrieval_tier ORDER BY composite_trust DESC"
---
