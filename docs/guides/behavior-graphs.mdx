...
section: "guides"
status: "stable"
  query: "MERGE (coord:Agent {id: 'coord', name: 'Coordinator', role: 'orchestrator', priority: 0, active: 1}) MERGE (planner:Agent {id: 'planner', name: 'Planner', role: 'planning', priority: 1, active: 1}) MERGE (executor:Agent {id: 'executor', name: 'Executor', role: 'execution', priority: 2, active: 1}) MERGE (guardian:Agent {id: 'guardian', name: 'Guardian', role: 'safety', priority: 0, active: 1}) MERGE (c:Agent {id: 'coord'})-[:DELEGATES_TO {on_event: 'detection', threshold: 0.85, priority: 1}]->(p:Agent {id: 'planner'}) MERGE (c:Agent {id: 'coord'})-[:DELEGATES_TO {on_event: 'hazard_alert', threshold: 0.70, priority: 0}]->(g:Agent {id: 'guardian'}) MERGE (p:Agent {id: 'planner'})-[:DELEGATES_TO {on_event: 'plan_ready', threshold: 0.80, priority: 2}]->(e:Agent {id: 'executor'}) MATCH (a:Agent)-[d:DELEGATES_TO]->(b:Agent) RETURN a.name AS from_agent, a.role, d.on_event, d.threshold AS confidence_gate, d.priority, b.name AS to_agent, b.role ORDER BY d.priority"
playground:
  query: "MERGE (coord:Agent {id: 'coord', name: 'Coordinator', role: 'orchestrator', priority: 0, active: 1})\n\nMERGE (planner:Agent {id: 'planner', name: 'Planner', role: 'planning', priority: 1, active: 1, on_event: 'detection', threshold: 0.85})\n\nMERGE (executor:Agent {id: 'executor', name: 'Executor', role: 'execution', priority: 2, active: 1, on_event: 'plan_ready', threshold: 0.80})\n\nMERGE (guardian:Agent {id: 'guardian', name: 'Guardian', role: 'safety', priority: 0, active: 1, on_event: 'hazard_alert', threshold: 0.70})\n\nMERGE (c1:Agent {id: 'coord'})-[:DELEGATES_TO]->(p:Agent {id: 'planner'})\n\nMERGE (c2:Agent {id: 'coord'})-[:DELEGATES_TO]->(g:Agent {id: 'guardian'})\n\nMERGE (p1:Agent {id: 'planner'})-[:DELEGATES_TO]->(e:Agent {id: 'executor'})\n\nMATCH (a:Agent)-[:DELEGATES_TO]->(b:Agent) RETURN a.name AS from_agent, a.role AS from_role, b.name AS to_agent, b.role AS to_role, b.on_event AS trigger_event, b.threshold AS confidence_gate, b.priority AS priority ORDER BY b.priority"
