...
playground:
  query: |-
    ...
    MATCH (c:Agent {id: 'coord'}) MATCH (p:Agent {id: 'planner'}) MERGE (c)-[:DELEGATES_TO {on_event: 'detection', threshold: 0.85, priority: 1}]->(p)
    MATCH (c:Agent {id: 'coord'}) MATCH (g:Agent {id: 'guardian'}) MERGE (c)-[:DELEGATES_TO {on_event: 'hazard_alert', threshold: 0.70, priority: 0}]->(g)
    MATCH (p:Agent {id: 'planner'}) MATCH (e:Agent {id: 'executor'}) MERGE (p)-[:DELEGATES_TO {on_event: 'plan_ready', threshold: 0.80, priority: 2}]->(e)
