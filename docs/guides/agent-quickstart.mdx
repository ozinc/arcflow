...
- **Algorithm output column name mismatch**: `CALL algo.pageRank()` returns columns `nodeId`, `name`, `labels`, `rank`. If `row.get('rank')` returns `undefined`, run `CALL db.help()` to inspect the current output schema.
...
- [Agent Governance](/docs/cli/governance) — verification state machine, receipts, and enforcement hooks
- [Plugin Management](/docs/cli/plugin) — extend the runtime with `arcflow plugin install`
