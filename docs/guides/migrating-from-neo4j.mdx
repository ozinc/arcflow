...
If you've worked with Neo4j, Memgraph, FalkorDB, TigerGraph, or Cosmos DB's Gremlin API, most of what you know transfers. ArcFlow's WorldCypher is Cypher-inspired; your existing `MATCH`, `CREATE`, `MERGE`, and `WHERE` clauses work without modification. What changes: the deployment model (in-process or lightweight server, no JVM), built-in GPU-accelerated algorithms, confidence scoring, temporal queries, and vector search with no add-ons.
...
| **Query language** | Cypher, Gremlin, GSQL, GQL | WorldCypher (ArcFlow's GQL dialect with spatial/temporal/reactive extensions) |
...
**Conformance:** ArcFlow passes 100% of the openCypher TCK (3881/3881 scenarios) and implements ISO GQL V2 fully, including GQLSTATUS codes, `IS LABELED`, `ELEMENT_ID`, `NEXT WHEN/THEN/ELSE/END` conditional pipelines, `UNIQUE` and `PRIMARY KEY` constraints, and transaction control statements.
