...
| **Query language** | Cypher, Gremlin, GSQL, openCypher | WorldCypher (Cypher-inspired with spatial/temporal primitives) |
...
## Cypher Dialect Compatibility

If you're coming from Neo4j, Memgraph, or FalkorDB (all openCypher-family), most of your queries port directly. Coming from TigerGraph (GSQL) or Cosmos DB (Gremlin) requires a query rewrite, but the graph modeling concepts transfer.

### Fully Compatible (openCypher family)
...
**Functions**: id(), labels(), type(), count(), sum(), avg(), min(), max(), collect(), coalesce(), size(), toString(), toInteger(), toFloat(), toLower(), toUpper(), trim(), substring(), left(), right(), replace(), split(), reverse()
...
Features in WorldCypher that don't exist in standard Cypher or other graph query languages:
...
Features in standard Cypher or other vendors not available in WorldCypher:
...
```sql
-- Most openCypher databases support exporting as CREATE statements
