...
playground:
  seed: world
  query: "MERGE (skill:Routine {id: 'entity-tracker', name: 'EntityTracker', version: 3, min_confidence: 0.82, active: 1}) MERGE (s1:Sensor {id: 'cam-1', name: 'Cam-LabA', type: 'camera', reliability: 0.95, confidence_floor: 0.85, poll_hz: 25}) MERGE (s2:Sensor {id: 'lidar-1', name: 'Lidar-LabB', type: 'lidar', reliability: 0.91, confidence_floor: 0.80, poll_hz: 10}) MERGE (agent:Agent {id: 'tracker-agent', name: 'TrackerPrime', role: 'tracking', active: 1}) MERGE (sk1:Routine {id: 'entity-tracker'})-[:APPLIED_TO]->(s1:Sensor {id: 'cam-1'}) MERGE (sk2:Routine {id: 'entity-tracker'})-[:APPLIED_TO]->(s2:Sensor {id: 'lidar-1'}) MERGE (ag:Agent {id: 'tracker-agent'})-[:EXECUTES]->(sk:Routine {id: 'entity-tracker'}) MATCH (a:Agent)-[:EXECUTES]->(sk:Routine)-[:APPLIED_TO]->(s:Sensor) RETURN a.name AS agent, sk.name AS skill, sk.version AS version, s.name AS sensor, s.type AS sensor_type, s.reliability AS reliability, s.confidence_floor AS min_confidence, s.poll_hz AS poll_rate ORDER BY s.poll_hz DESC"
canonical: skills
---
...
- [Concepts: Skills](/docs/skills) — what skills are, introspection (`arcflow.skills()`), and provenance (`db.provenance()`)
- [CREATE SKILL](/skills/create-skill) — full syntax reference
- [PROCESS NODE](/skills/process-node) — query-time skill invocation with optional WHERE filtering
...
- [Agent Governance](/docs/cli/governance) — verification state machine and tamper-evident receipts
