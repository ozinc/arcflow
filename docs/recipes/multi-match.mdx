...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH s, d, r, z, s.reliability * d.confidence AS composite_trust, (r.x - s.x) * (r.x - s.x) + (r.y - s.y) * (r.y - s.y) AS range_sq RETURN s.name AS sensor, s.type, d.method AS detection_method, composite_trust, range_sq, r.name AS robot, r.task, r.battery, z.name AS zone, z.hazard ORDER BY composite_trust DESC"
---
