...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WHERE s.reliability >= 0.90 WITH z.name AS zone, z.hazard AS hazard, count(r) AS grounded_entities, avg(d.confidence) AS mean_confidence, avg(s.reliability) AS mean_reliability, avg(s.reliability * d.confidence) AS composite_trust, min(s.reliability * d.confidence) AS min_trust RETURN zone, hazard, grounded_entities, mean_confidence, mean_reliability, composite_trust, min_trust, CASE WHEN composite_trust >= 0.93 THEN 'tier1_verified' WHEN composite_trust >= 0.88 THEN 'tier2_operational' ELSE 'tier3_uncertain' END AS rag_tier ORDER BY composite_trust DESC"
...
- [Vector Search](/vector-search) — vector index setup and hybrid search configuration
