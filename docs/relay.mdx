...
status: "stable"
playground:
  query: "MERGE (ctrl:Agent {id: 'ctrl', name: 'ControlCenter', role: 'hub', hops: 0}) MERGE (fa:Agent {id: 'field-a', name: 'FieldAgent-Alpha', role: 'field', lat_ms: 12, hops: 1, zone: 'lab-a', intent: 'patrol', priority: 1, channel: 'primary', reliability: 0.99}) MERGE (fb:Agent {id: 'field-b', name: 'FieldAgent-Beta', role: 'field', lat_ms: 14, hops: 1, zone: 'lab-b', intent: 'inspect', priority: 2, channel: 'primary', reliability: 0.99}) MERGE (sa:Agent {id: 'sub-a', name: 'SubAgent-A1', role: 'subagent', lat_ms: 28, hops: 2, zone: 'lab-a', intent: 'monitor', priority: 1, channel: 'local', reliability: 0.95}) MERGE (c1:Agent {id: 'ctrl'})-[:RELAYS_TO]->(fa:Agent {id: 'field-a'}) MERGE (c2:Agent {id: 'ctrl'})-[:RELAYS_TO]->(fb:Agent {id: 'field-b'}) MERGE (fa1:Agent {id: 'field-a'})-[:RELAYS_TO]->(sa:Agent {id: 'sub-a'}) MATCH (src:Agent)-[:RELAYS_TO]->(dst:Agent) RETURN src.name AS from_agent, src.role AS from_role, dst.name AS to_agent, dst.role AS to_role, dst.intent AS intent, dst.priority AS priority, dst.channel AS channel, dst.reliability AS reliability, dst.lat_ms AS latency_ms, dst.hops AS relay_hops ORDER BY dst.hops, dst.priority"
