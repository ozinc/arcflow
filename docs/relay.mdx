...
playground:
  query: |-
    ...
    MATCH (ctrl:Agent {id: 'ctrl'}), (fa:Agent {id: 'field-a'}) MERGE (ctrl)-[:RELAYS_TO {intent: 'patrol', priority: 1, channel: 'primary', reliability: 0.99}]->(fa)

    MATCH (ctrl:Agent {id: 'ctrl'}), (fb:Agent {id: 'field-b'}) MERGE (ctrl)-[:RELAYS_TO {intent: 'inspect', priority: 2, channel: 'primary', reliability: 0.99}]->(fb)

    MATCH (fa:Agent {id: 'field-a'}), (sa:Agent {id: 'sub-a'}) MERGE (fa)-[:RELAYS_TO {intent: 'monitor', priority: 1, channel: 'local', reliability: 0.95}]->(sa)
