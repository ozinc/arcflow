...
playground:
  query: |-
    ...
    MERGE (ctrl:Agent {id: 'ctrl'})-[:RELAYS_TO {intent: 'patrol', priority: 1, channel: 'primary', reliability: 0.99}]->(fa:Agent {id: 'field-a'})

    MERGE (ctrl:Agent {id: 'ctrl'})-[:RELAYS_TO {intent: 'inspect', priority: 2, channel: 'primary', reliability: 0.99}]->(fb:Agent {id: 'field-b'})

    MERGE (fa:Agent {id: 'field-a'})-[:RELAYS_TO {intent: 'monitor', priority: 1, channel: 'local', reliability: 0.95}]->(sa:Agent {id: 'sub-a'})
