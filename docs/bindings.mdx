...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH s, d.confidence AS confidence, r, z, s.reliability * d.confidence AS composite_trust RETURN s.name AS sensor, confidence, composite_trust, r.name AS robot, r.task AS task, r.battery AS battery, z.name AS zone ORDER BY composite_trust DESC"
---
