...
Scratch base image, statically linked binary, under 20MB. NVIDIA bare-metal
strategy — the container adds zero runtime overhead.

The published image and `docker run` command come from the install matrix on
the [Installation page](/installation). Once installed, mount your data
directory:

```bash
# Replace <image> with the docker row from the install matrix.
docker run -v /data/arcflow:/data <image> --data-dir /data
```
