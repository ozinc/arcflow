---
title: "World Graph Substrate (Engine Preview)"
kind: operation
description: "Engine-architecture preview of the World Graph substrate — module structure, virtual labels, six-tier residency, oz:// URI scheme, and the ARC1 on-disk format. Type vocabulary is frozen; implementation lands progressively."
status: "preview"
---

# World Graph Substrate (Engine Preview)

This page is an **engine-architecture preview**, not a user-facing capability reference. It describes the substrate the World Graph layer is being built on — the module structure, the type vocabulary, the storage hierarchy, the addressing scheme, and the on-disk format. The types are frozen and exhaustively tested; the implementation that animates them lands progressively as the substrate cuts.

When an external surface is documented elsewhere (DDL, procedures, the SDK), this page links to it. When a substrate concept does not yet have a user-facing surface, this page describes the concept and does not invent one. *Live, not aspirational* — every claim here either describes types that exist in the engine source today, or marks a downstream surface that arrives when the substrate's resolvers wire through.
...
## Status — what is frozen, what is in flight

The type vocabulary in the engine source today:

- `arcflow.worldgraph` module skeleton with the six bounded capabilities + `io` substrate.
- Typed schema (`NodeLabel`, `ColumnDef`, `ColumnType`, `EdgeLabel`).
- Virtual-label contracts (`VirtualLabelEntry`, `PartitionPattern`, `ResolverKind`).
- DDL parser for `CREATE NODE LABEL ... VIRTUAL FROM PARTITION`.
- `MutationOp` enum (row-level and bulk-stripe variants).
- WAL record envelope and bulk-stripe protocol.
- `TierBudget` + nine-state `ResidencyClass`.
- `oz://` URI parser.
- ARC1 file magic + version constants.
- LSM compaction policy types.
- Streaming-stripe writer policy + state machine.
- Block-cache key + handle + policy types.
- Catalog skeleton.
- Platform probe (macOS / Linux / WSL2) with capability dispatch.

In flight — the page does not describe these as live:

- Streaming-stripe write bodies, manifest replay + snapshot pinning.
- Object-cache fetch bodies, Parquet decoder.
- Apply-mutation row-store wiring, column-store lift, MVCC, dense-store.
- WAL append + replay bodies.
- Level-compaction scheduler + bandwidth governor.
- Memory Governor enforcement body.
- ARC1 reader + Parquet decoder paths.
- `oz://` resolvers + fsspec binding.

When these wire through, the corresponding user-facing surfaces become discoverable through the SDK + DDL + procedure references. Until they do, this preview is the canonical description.
