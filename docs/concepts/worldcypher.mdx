---
title: "WorldCypher Query Language"
description: "WorldCypher Query Language"
section: "concepts"
status: "stable"
---
...
WorldCypher is ArcFlow's GQL dialect — an implementation of [ISO/IEC 39075 GQL](https://www.iso.org/standard/76120.html), the international standard for graph query languages. The syntax is compatible with Cypher, so existing Cypher queries run without modification. WorldCypher extends GQL with temporal queries (`AS OF`), reactive queries (`LIVE MATCH`, `CREATE LIVE VIEW`), vector search, graph algorithms, and confidence/provenance semantics.
