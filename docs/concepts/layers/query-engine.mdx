...
The planner picks a compute lane automatically for each algorithm call — `cpu_scalar`, `cpu_simd`, `gpu_cuda`, `gpu_metal`, or `auto`. A `HINT` clause overrides that choice:

```cypher
CALL algo.pageRank() HINT lane=gpu_cuda YIELD nodeId, score
CALL algo.triangleCount() HINT lane=cpu_simd YIELD count
```

If the requested lane is unavailable on the host (e.g. `gpu_cuda` on Apple Silicon), the engine falls back silently to the next-best lane and records the actual lane used. The chosen lane comes back on the result envelope as `result.transport_outcome.lane` — use it to detect silent fallbacks in tests, or to log which path each query actually took.
