...
> **Information-theoretic view:** `confidence` is the probability a named model assigned to a fact, so the fact's *information content* is `−log₂ confidence` — a low-confidence-but-true fact carries the most bits. That makes the evidence field literally Shannon information, callable as `CALL arcflow.info.nodeSurprisal(id)`. See [the Information Layer](/docs/concepts/information-layer).
