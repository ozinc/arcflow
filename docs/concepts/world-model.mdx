...
The neural world model research community is trying to collapse simulation, memory, spatial grounding, and calibrated uncertainty into a single generative model. MosaicMem says "spatial memory is the bottleneck." PERSIST argues for "a persistent latent 3D scene." Papers are being published about adding uncertainty awareness to neural WFMs. This direction is architecturally wrong.

**ArcFlow is the correct architectural separation.** Simulation belongs in the neural layer — learned, generative, action-conditioned. Persistence belongs in a separate, deterministic, queryable layer. These are not the same problem. Mixing them produces systems that are worse at both.

Two distinct things go by the name "world model" today. Getting them confused — or trying to merge them — is costly.

## The two definitions

**Neural world model** — a neural network trained to simulate how the world evolves under actions. Given an observation and an action, it predicts the next state. Action-conditioned, generative, trained on petabytes of sensor data. Architecturally: a Vision model compresses observations into latent space, a Memory model (RSSM / MDN-RNN) predicts how latents evolve given actions, a Controller selects actions — together they let agents rehearse inside a hallucinated future before acting in reality. This is the sense used by the leading neural world model platforms in research and production. These systems simulate. They are not databases. They do not store what actually happened. They cannot tell you where Robot-04 was at 14:23:07 with confidence 0.91 from lidar-03.

**Operational world model** — a persistent, queryable store of actual world state. What entities exist, where they are, how they relate to each other, how confident we are in each fact, what the world looked like at any previous moment. This is ArcFlow. It records. It does not hallucinate — it stores neural model outputs as they arrive (`_observation_class: 'predicted'`), alongside sensor-confirmed observations and inferred facts, all versioned and spatially indexed.

These two layers are architecturally distinct but operationally complementary. Neural world models hallucinate possible futures → their outputs land in ArcFlow as queryable predicted state. ArcFlow's grounded observations feed back as the actual truth the neural model learns to predict from. One simulates, one records.

**ArcFlow is the operational world model layer.** Not a simulation sandbox — a factual one.

The right test isn't "which layer does more?" — it's asking each layer the questions it was built to answer:

```cypher
-- A question no neural world model can answer:
...
This runs in ~0.1ms. Deterministically. Against exact sensor history. Neural world models cannot answer it — they simulate plausible futures, not factual pasts.

| Operational question | Neural world model | ArcFlow |
|---|---|---|
| What did sensor-3 detect at seq 14,207 with confidence > 0.9? | Cannot answer | `AS OF seq 14207` — instant |
| Which entity triggered the hazard alert, and which sensor saw it? | Cannot answer | Multi-hop graph traversal — exact |
| What is the confidence-weighted state of Zone-B right now? | Cannot answer | Live view — sub-millisecond |
| How has this entity's trajectory changed over the last 5 minutes? | Cannot answer | `AS OF seq N` comparison — deterministic |
| Which agents are within 20m of a high-confidence obstacle? | Cannot answer | `algo.nearestNodes()` — ~0.1ms |
| Is this observation class `observed` or `predicted`? | Not modeled | `_observation_class` on every node/edge |
| What was the confidence score, and from which sensor? | Not stored | `_confidence` + provenance edge — always |

Neural world models score zero on every row. These aren't edge cases — they're the operational baseline for any production robotic, fleet, or multi-agent system.

The frontier of neural world modeling is converging on this gap. MosaicMem (2025): "spatial memory is the bottleneck for controllable world simulators." PERSIST: "interactive world models need a persistent latent 3D scene, not just pixel histories." These papers are trying to *learn* persistence — adding it into the generative model itself, approximate and expensive. ArcFlow's thesis is that persistence belongs in a separate, deterministic, queryable layer: exact, free, and already solved.
