...
The digital world has been queryable for thirty years — every click, transaction, and event captured, indexed, and ready to answer *what happened, what is happening, what comes next*. The operational world — a venue, a warehouse, a robot fleet, a live match — has remembered almost nothing. The event happens and the system forgets: raw video that needs human eyes, point-in-time dashboards, state overwritten on every update, history gone the moment it passes.

ArcFlow closes that gap. It gives the operational world a **persistent, queryable memory**: a structured, versioned, spatially-grounded record of what is actually true now, and what was true at any past moment. That is what we mean by *world model* — and the term carries two meanings, so the difference is worth making precise.
...
Deterministic. Against exact sensor history.

| Question | ArcFlow |
|---|---|
| What did sensor-3 detect at seq 14,207 with confidence > 0.9? | `AS OF seq 14207` |
| Which entity triggered the hazard alert, and which sensor saw it? | Multi-hop graph traversal — exact |
| What is the confidence-weighted state of Zone-B right now? | Live view |
| How has this entity's trajectory changed over the last 5 minutes? | `AS OF seq N` comparison — deterministic |
| Which agents are within 20m of a high-confidence obstacle? | `algo.nearestNodes()` |
...
|---|---|---|
| **Space** | Coordinates as numeric columns | Spatial predicates native — spatially indexed, frustum queries, proximity algorithms |
...
- [Grounded Neural Objects](/docs/use-cases/grounded-neural-objects) — lifting neural world model outputs into persistent ArcFlow entities
- [Autonomous Systems](/docs/use-cases/autonomous-systems) — the two-layer stack applied to robot fleets and UAV coordination
...
- [Spatial Queries](/spatial-knowledge) — spatial primitives deep dive
