...
Neural world models simulate how the world evolves under actions — generative, action-conditioned, trained on petabytes of sensor data to predict possible futures. Persistence is a separate concern: a deterministic, queryable record of what actually happened, spatially indexed and versioned to the sequence number. These two layers serve different purposes, operate at different latency tiers, and are built on fundamentally different foundations.

**ArcFlow is the persistence layer.** Simulation belongs in the neural tier. ArcFlow records what the simulation produces — and everything else the world sends in.
