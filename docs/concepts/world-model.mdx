...
A simple test separates the two layers:

| Question | Neural world model | ArcFlow |
|---|---|---|
| Can I **intervene** and see what changes? | Yes — action-conditioned | Yes — `MUTATE`, confidence propagates |
| Does it **remember** when the camera moves away? | Sometimes (MosaicMem, PERSIST) | Always — full versioned history |
| Can it roll out **counterfactual futures**? | Yes — its core purpose | No — not its job |
| Can I use it for **planning/evaluation**? | Yes — dream-based RL | Yes — query observed state to inform decisions |
| Does it know when it is **uncertain**? | Some (calibrated WMs) | Always — `_confidence` and `_observation_class` on every fact |

The frontier of neural world modeling is converging on persistence and grounded spatial memory (MosaicMem: "spatial memory is the bottleneck"; PERSIST: "interactive world models need a persistent latent 3D scene, not just pixel histories"). What those papers are trying to *learn*, ArcFlow stores as *ground truth* — for the actual world, from actual sensors, at factual precision.
