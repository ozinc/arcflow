...
The current AI infrastructure stack has three tiers. Foundation models handle language and perception. Vector databases handle similarity search. And operational world models — the tier that has been missing — handle **what the world is, was, and will be**: spatial precision, temporal fidelity, epistemic honesty about what is known versus inferred.

Neural world models (Cosmos, FSD, Genie) sit at the top — generative, petabyte-trained, action-predicated. They are extraordinary at simulating possible futures. They are not designed to remember that Robot-04 entered Zone-B at 14:23:07 with confidence 0.91, or that the ball crossed the goal line 200ms before the keeper moved.

ArcFlow is the middle tier — the persistence layer that stores what actually happened, tagged with who observed it, at what confidence, from which sensor, at what sequence checkpoint. It is what neural world models write their outputs into (`_observation_class: 'predicted'`), and what they read ground truth from.

An autonomous vehicle fleet needs to know where every vehicle is, where it was, and what confidence level its sensor data carries — not retrieved from a language model, not approximated by vector similarity, but tracked precisely in a spatial-temporal graph. A multi-agent AI system needs persistent shared state that every agent reads from and writes to, with full provenance of who changed what and when. A physical AI deployment needs a live replica of its environment that can be queried, replayed, and reasoned over.

That is ArcFlow: the operational world model layer that foundation models, neural world models, agent frameworks, and reasoning systems build on top of.
