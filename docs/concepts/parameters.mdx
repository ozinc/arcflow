...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WHERE r.battery >= 60 AND d.confidence >= 0.85 WITH s, d, r, z, s.reliability * d.confidence AS composite_trust RETURN r.name AS robot, r.task, r.battery AS energy_pct, d.confidence AS detection_confidence, composite_trust, s.name AS sensor, s.type, z.name AS zone, z.hazard ORDER BY composite_trust DESC"
---
