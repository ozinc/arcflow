---
title: "Query Results"
description: "World model query results as typed rows — numbers are numbers, not strings. ISO GQL status codes, mutation statistics, and structured access patterns."
status: "stable"
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS hazard, count(r) AS entity_count, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, hazard, entity_count, avg_battery, composite_trust, CASE WHEN composite_trust >= 0.9 THEN 'fully-observed' WHEN composite_trust >= 0.75 THEN 'partially-observed' ELSE 'low-confidence' END AS observability ORDER BY composite_trust DESC"
---

# Query Results

How to work with data returned from queries.

## QueryResult structure

Every `db.query()` and `db.mutate()` returns a result with:

```typescript
interface QueryResult {
  columns: string[]      // Column names in result order
  rows: TypedRow[]       // Result rows with typed access
  rowCount: number       // Number of rows
  computeMs: number      // Execution time in milliseconds
  gqlstatus(): string    // ISO GQL status: "00000" = data, "02000" = no data
  snapshotUri: string    // arcflow://snapshot/<hex> — the snapshot this query observed
}
```

The `snapshotUri` is content-addressed and round-trippable: pass it to `db.queryAt(uri,
cypher)` to replay the same query against the same world state and get bit-for-bit
identical rows. See [Snapshot-Pinned Reads](/concepts/snapshots).

## Accessing values

### By column name (recommended)

```typescript
const result = db.query("MATCH (n:Person) RETURN n.name, n.age")

for (const row of result.rows) {
  const name = row.get('name')   // string
  const age = row.get('age')     // number (automatically typed)
  console.log(name, age)
}
```

Column names match the query's RETURN clause. For `RETURN n.name`, the column is `n.name` — but the SDK also indexes the short form `name`, so both work:

```typescript
row.get('n.name')  // works
row.get('name')    // also works (short form)
```

### As a full object

```typescript
const obj = row.toObject()
// { 'n.name': 'Alice', 'n.age': 30, 'n.active': true }
```

### Iterate all rows

```typescript
const result = db.query("MATCH (n:Person) RETURN n.name, n.age")
const people = result.rows.map(row => row.toObject())
// [{ 'n.name': 'Alice', 'n.age': 30 }, { 'n.name': 'Bob', 'n.age': 35 }]
```

## Automatic type coercion

The SDK automatically parses raw string values into their correct TypeScript types:

| Raw Value | TypeScript Type | Returned As |
|---|---|---|
| `'Alice'` | `string` | `'Alice'` |
| `'30'` | `number` | `30` |
| `'3.14'` | `number` | `3.14` |
| `'true'` | `boolean` | `true` |
| `'false'` | `boolean` | `false` |
| `''` or `'null'` | `null` | `null` |

## MutationResult

`db.mutate()` returns a `MutationResult` which extends `QueryResult` with mutation statistics:

```typescript
interface MutationResult extends QueryResult {
  nodesCreated: number
  nodesDeleted: number
  relationshipsCreated: number
  relationshipsDeleted: number
  propertiesSet: number
}
```

## Checking empty results

```typescript
const result = db.query("MATCH (n:Missing) RETURN n")
if (result.rowCount === 0) {
  console.log('No results found')
}
```

## ISO GQL status codes

`gqlstatus()` returns the ISO/IEC 39075 GQL status for the result:

```typescript
const result = db.query("MATCH (n:Person) WHERE n.age > 100 RETURN n.name")
result.gqlstatus()  // "02000" — query succeeded, no matching rows
// vs.
const result2 = db.query("MATCH (n:Person) RETURN n.name")
result2.gqlstatus() // "00000" — rows were returned
```

| Code | Meaning |
|---|---|
| `"00000"` | Success with data — at least one row returned |
| `"02000"` | No data — query executed correctly but matched nothing |

## Timing

```typescript
const result = db.query("CALL algo.pageRank()")
console.log(`PageRank computed in ${result.computeMs.toFixed(1)}ms`)
```

---

## See Also

- [Parameters](/concepts/parameters) — always use `$params` placeholders for world model queries
- [TypeScript API Reference](/reference/api) — full method signatures and return types
- [Error Handling](/concepts/error-handling) — what `db.query()` throws and when
- [Snapshot-Pinned Reads](/concepts/snapshots) — how `snapshotUri` enables provenanced re-runs
