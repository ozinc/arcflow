...
playground:
  seed: world
  query: |-
    MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS zone_risk, count(r) AS robot_count, avg(s.reliability * d.confidence) AS avg_composite_trust, avg(r.battery) AS avg_energy RETURN zone, zone_risk, robot_count, avg_composite_trust, avg_energy ORDER BY avg_composite_trust DESC
