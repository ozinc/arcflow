...
```cypher
-- All players within 3m of the ball, 10 seconds before the goal was scored
MATCH (g:Goal {match_id: 'match-001'})
MATCH (b:Ball) AS OF (g.timestamp - 10000)
  WHERE distance(b.position, g.position) < 1.0
MATCH (p:Player) AS OF (g.timestamp - 10000)
  WHERE distance(p.position, b.position) < 3.0
RETURN p.name, p.team, p._confidence
ORDER BY p._confidence DESC
```

The world model at `g.timestamp - 10000` is the exact graph state 10 seconds before the goal. No log reconstruction. No event replay pipeline. The same query syntax, a different time.
...
```cypher
-- Facts that were current 24h ago but have since been superseded
MATCH (f:Fact) AS OF (timestamp() - 86400000)
