...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH s, d.method AS method, r, z, s.reliability * d.confidence AS perception_quality, r.battery * d.confidence AS operational_score RETURN s.name AS sensor, s.type AS modality, perception_quality, method, r.name AS robot, r.task AS task, r.battery AS battery, operational_score, z.name AS zone, z.hazard AS hazard ORDER BY operational_score DESC"
---
