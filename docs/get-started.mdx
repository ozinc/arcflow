...
title: "Get Started"
kind: first-use
description: "ArcFlow is the blazing-fast graph engine for modeling the real world — the persistence tier for spatial-temporal, confidence-scored world state. Neural world models simulate. ArcFlow records."
status: "stable"
playground:
  seed: world
...
Deterministic. Against exact history. ArcFlow tells you what *did* happen — which sensor, which detection, what confidence, which zone, at any moment in the past — with one query.
...
Open **[ArcFlow Engine](/engine)** in your browser. Paste any GQL query — persistent storage, no install, no signup.
...
The full install matrix — every binding, with current shipping status — is on
the [Installation page](/installation). It is generated from the engine's
release manifest, so what you see is what is actually published.

For the impatient:

- **[Browser](/engine)** — zero install, click and go.
- **CLI (coding agents)** — once installed, `arcflow query '...'`.
...
- [Project Setup](/docs/bindings) — Express, testing, monorepo
...
- [Agent Integration](/docs/guides/agent-quickstart) — CLI binary, MCP, and napi-rs — which surface fits your execution context
...
- [Digital Twins](/docs/use-cases/digital-twins) — live spatial replica of physical facilities
- [Robotics & Perception](/markets/robotics) — sensor fusion, track lifecycle, ROS bridge
