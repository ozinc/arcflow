...
playground:
  query: |-
    MERGE (c1:Concept {id: 'world-model', name: 'World Model', domain: 'AI', maturity: 'production', complexity: 0.9})
    ...
    MATCH (c1:Concept {id: 'world-model'}) MATCH (c2:Concept {id: 'spatial-graph'}) MERGE (c1)-[:BUILT_ON {strength: 0.95, required: 1}]->(c2)
    MATCH (c1:Concept {id: 'world-model'}) MATCH (c3:Concept {id: 'conf-scoring'}) MERGE (c1)-[:BUILT_ON {strength: 0.88, required: 0}]->(c3)
    MATCH (c1:Concept {id: 'world-model'}) MATCH (c4:Concept {id: 'temporal-query'}) MERGE (c1)-[:BUILT_ON {strength: 0.91, required: 1}]->(c4)
    MATCH (c5:Concept {id: 'causal-graph'}) MATCH (c1:Concept {id: 'world-model'}) MERGE (c5)-[:BUILT_ON {strength: 0.82, required: 1}]->(c1)
    MATCH (src:Concept)-[r:BUILT_ON]->(dep:Concept) RETURN src.name AS concept, src.domain, src.maturity, src.complexity, r.strength AS dependency_strength, r.required, dep.name AS depends_on, dep.domain AS dep_domain, dep.maturity AS dep_maturity ORDER BY r.strength DESC
