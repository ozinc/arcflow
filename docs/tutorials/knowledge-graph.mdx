...
status: "stable"
playground:
  query: "MERGE (c1:Concept {id: 'world-model', name: 'World Model', domain: 'AI', maturity: 'production', complexity: 0.9}) MERGE (c2:Concept {id: 'spatial-graph', name: 'Spatial Graph', domain: 'databases', maturity: 'production', complexity: 0.7}) MERGE (c3:Concept {id: 'conf-scoring', name: 'Confidence Scoring', domain: 'AI', maturity: 'emerging', complexity: 0.6}) MERGE (c4:Concept {id: 'temporal-query', name: 'Temporal Query', domain: 'databases', maturity: 'stable', complexity: 0.8}) MERGE (c5:Concept {id: 'causal-graph', name: 'Causal Graph', domain: 'AI', maturity: 'research', complexity: 0.95}) MERGE (c1:Concept {id: 'world-model'})-[:BUILT_ON]->(c2:Concept {id: 'spatial-graph'}) MERGE (c1a:Concept {id: 'world-model'})-[:BUILT_ON]->(c3:Concept {id: 'conf-scoring'}) MERGE (c1b:Concept {id: 'world-model'})-[:BUILT_ON]->(c4:Concept {id: 'temporal-query'}) MERGE (c5a:Concept {id: 'causal-graph'})-[:BUILT_ON]->(c1c:Concept {id: 'world-model'}) MATCH (src:Concept)-[:BUILT_ON]->(dep:Concept) RETURN src.name AS concept, src.domain AS domain, src.maturity AS maturity, src.complexity AS complexity, dep.name AS depends_on, dep.domain AS dep_domain, dep.maturity AS dep_maturity ORDER BY src.complexity DESC"
