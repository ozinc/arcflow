...
playground:
  query: |-
    ...
    MERGE (c1:Concept {id: 'world-model'})-[:BUILT_ON {strength: 0.95, required: 1}]->(c2:Concept {id: 'spatial-graph'})

    MERGE (c1:Concept {id: 'world-model'})-[:BUILT_ON {strength: 0.88, required: 0}]->(c3:Concept {id: 'conf-scoring'})

    MERGE (c1:Concept {id: 'world-model'})-[:BUILT_ON {strength: 0.91, required: 1}]->(c4:Concept {id: 'temporal-query'})

    MERGE (c5:Concept {id: 'causal-graph'})-[:BUILT_ON {strength: 0.82, required: 1}]->(c1:Concept {id: 'world-model'})
