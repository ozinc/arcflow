...
playground:
  query: |-
    ...
    MATCH (c1:Concept {id: 'world-model'}), (c2:Concept {id: 'spatial-graph'}) MERGE (c1)-[:BUILT_ON {strength: 0.95, required: true}]->(c2)
    MATCH (c1:Concept {id: 'world-model'}), (c3:Concept {id: 'conf-scoring'}) MERGE (c1)-[:BUILT_ON {strength: 0.88, required: false}]->(c3)
    MATCH (c1:Concept {id: 'world-model'}), (c4:Concept {id: 'temporal-query'}) MERGE (c1)-[:BUILT_ON {strength: 0.91, required: true}]->(c4)
    MATCH (c5:Concept {id: 'causal-graph'}), (c1:Concept {id: 'world-model'}) MERGE (c5)-[:BUILT_ON {strength: 0.82, required: true}]->(c1)
