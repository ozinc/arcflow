...
playground:
  query: |-
    ...
    MATCH (c1:Concept {id: 'world-model'}), (c2:Concept {id: 'spatial-graph'}) MERGE (c1)-[:BUILT_ON {strength: 0.95, required: 1}]->(c2)

    MATCH (c1:Concept {id: 'world-model'}), (c3:Concept {id: 'conf-scoring'}) MERGE (c1)-[:BUILT_ON {strength: 0.88, required: 0}]->(c3)

    MATCH (c1:Concept {id: 'world-model'}), (c4:Concept {id: 'temporal-query'}) MERGE (c1)-[:BUILT_ON {strength: 0.91, required: 1}]->(c4)

    MATCH (c5:Concept {id: 'causal-graph'}), (c1:Concept {id: 'world-model'}) MERGE (c5)-[:BUILT_ON {strength: 0.82, required: 1}]->(c1)
