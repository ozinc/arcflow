...
playground:
  query: |-
    MERGE (da:Document {id: 'doc-wm', title: 'World Model Architecture', category: 'graph', relevance: 0.95, tokens: 1420})
    ...
    MATCH (a:Document {id: 'doc-wm'}) MATCH (b:Document {id: 'doc-sp'}) MERGE (a)-[:RELATED_TO {strength: 0.92}]->(b)
    MATCH (a:Document {id: 'doc-wm'}) MATCH (b:Document {id: 'doc-conf'}) MERGE (a)-[:RELATED_TO {strength: 0.85}]->(b)
    MATCH (a:Document {id: 'doc-conf'}) MATCH (b:Document {id: 'doc-rag'}) MERGE (a)-[:RELATED_TO {strength: 0.88}]->(b)
    MATCH (doc:Document)-[r:RELATED_TO]->(related:Document) WHERE doc.relevance > 0.78 RETURN doc.title AS document, doc.category, doc.relevance AS vector_score, doc.tokens, related.title AS related_document, r.strength AS graph_strength, doc.relevance * r.strength AS combined_score ORDER BY combined_score DESC
