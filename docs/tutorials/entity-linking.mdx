...
playground:
  query: |-
    ...
    MATCH (robot:Robot {id: 'r1'}), (obs_cam:Observation {id: 'obs-cam-r1'}) MERGE (obs_cam)-[:RESOLVES_TO {method: 'vision-match', verified: 1, strength: 0.96}]->(robot)

    MATCH (robot:Robot {id: 'r1'}), (obs_lidar:Observation {id: 'obs-lidar-r1'}) MERGE (obs_lidar)-[:RESOLVES_TO {method: 'geometry-match', verified: 0, strength: 0.89}]->(robot)

    MATCH (robot:Robot {id: 'r1'}), (obs_rfid:Observation {id: 'obs-rfid-r1'}) MERGE (obs_rfid)-[:RESOLVES_TO {method: 'rfid-tag', verified: 1, strength: 0.99}]->(robot)
