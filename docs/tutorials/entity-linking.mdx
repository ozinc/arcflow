...
playground:
  query: |-
    MERGE (robot:Robot {id: 'r1', name: 'Atlas-01', canonical: true, x: 18.0, y: 8.5, battery: 87, task: 'assembly'})
    ...
    MATCH (robot:Robot {id: 'r1'}), (obs_cam:Observation {id: 'obs-cam-r1'}) MERGE (obs_cam)-[:RESOLVES_TO {method: 'vision-match', verified: true, strength: 0.96}]->(robot)
    MATCH (robot:Robot {id: 'r1'}), (obs_lidar:Observation {id: 'obs-lidar-r1'}) MERGE (obs_lidar)-[:RESOLVES_TO {method: 'geometry-match', verified: false, strength: 0.89}]->(robot)
    MATCH (robot:Robot {id: 'r1'}), (obs_rfid:Observation {id: 'obs-rfid-r1'}) MERGE (obs_rfid)-[:RESOLVES_TO {method: 'rfid-tag', verified: true, strength: 0.99}]->(robot)
