...
status: "stable"
playground:
  query: "MERGE (sk:Routine {id: 'prox-detect', name: 'ProximityDetector', version: 4, radius: 30.0, active: 1}) MERGE (s1:Sensor {id: 'cam-1', name: 'Cam-LabA', type: 'camera', reliability: 0.95, trust_gate: 0.85, poll_hz: 25}) MERGE (s2:Sensor {id: 'lidar-1', name: 'Lidar-LabB', type: 'lidar', reliability: 0.91, trust_gate: 0.80, poll_hz: 10}) MERGE (s3:Sensor {id: 'rfid-1', name: 'RFID-Gate', type: 'rfid', reliability: 0.99, trust_gate: 0.95, poll_hz: 1}) MERGE (sk1:Routine {id: 'prox-detect'})-[:GOVERNS]->(s1:Sensor {id: 'cam-1'}) MERGE (sk2:Routine {id: 'prox-detect'})-[:GOVERNS]->(s2:Sensor {id: 'lidar-1'}) MERGE (sk3:Routine {id: 'prox-detect'})-[:GOVERNS]->(s3:Sensor {id: 'rfid-1'}) MATCH (sk:Routine)-[:GOVERNS]->(s:Sensor) RETURN sk.name AS skill, sk.version AS version, sk.radius AS detection_radius, s.name AS sensor, s.type AS sensor_type, s.reliability AS reliability, s.trust_gate AS trust_gate, s.poll_hz AS poll_hz ORDER BY s.poll_hz DESC"
---
...
A skill is defined from a **rule source** and runs at one of four **tiers**. Two sources:

```cypher
-- From a natural-language rule (Symbolic pattern, or LLM judgment)
CREATE SKILL <name>
  FROM PROMPT '<rule_or_instruction>'
  ALLOWED ON [<Label>, ...]
  TIER SYMBOLIC | LLM | WASM
  [INPUT VIEW <view_name>]          -- optional: scope the skill's input to a live view
  [MODEL '<provider>/<model>']      -- optional (LLM tier): pin the model, e.g. 'cli/claude-code'

-- From embedding similarity (Neural tier)
CREATE SKILL <name>
  FROM EMBEDDING <property | arcflow.cosine(source.prop, target.prop)>
  THRESHOLD <0..1>
  ALLOWED ON [<Label>, ...]
  TIER NEURAL
```
...
| Clause | Meaning |
|---|---|
| `FROM PROMPT '<rule>'` | Natural-language rule — pattern-matched (Symbolic), executed in WASM, or judged by an LLM, per `TIER`. |
| `FROM EMBEDDING <expr> THRESHOLD <f>` | Link nodes whose embedding similarity ≥ threshold (Neural tier). |
| `ALLOWED ON [Labels]` | The labels the skill may run against. |
| `TIER SYMBOLIC \| NEURAL \| LLM \| WASM` | The execution tier (see [Skills](/docs/skills)). |
| `INPUT VIEW <view>` | Optional — restrict the skill's input to a [live view](/docs/worldcypher/statements/live-views). |
| `MODEL '<provider>/<model>'` | Optional, LLM tier — pin the model from the provider catalog. |
...
- [Concepts: Skills](/docs/skills) — the full skills model explained
