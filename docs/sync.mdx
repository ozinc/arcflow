...
| **Arrow Flight WAL tailing** | High-throughput WAL streaming to read replicas |
| **Object-store fan-out** | WAL segments written to S3/GCS/Azure for durable replication |
| WASM (browser) | Not yet supported |

## Replication (SWMR)

ArcFlow supports a Single-Writer-Multiple-Reader replication model via Arrow Flight WAL tailing. The primary node streams WAL entries over Arrow Flight; read replicas apply them in order.
...
-- Arrow Flight WAL tailing parameters
...
- [Cloud Architecture](/cloud-architecture) — distributed mesh, Arrow Flight WAL tailing, multi-region
