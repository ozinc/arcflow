...
title: "Sync"
kind: capability
description: "Local-first synchronization of the persistent world model — spatial positions, temporal history, and confidence scores all replicate to cloud automatically with conflict resolution"
status: "stable"
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS hazard, count(r) AS entity_count, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS sync_quality RETURN zone, hazard, entity_count, avg_battery, sync_quality, CASE WHEN sync_quality >= 0.90 THEN 'synced' WHEN sync_quality >= 0.75 THEN 'partial' ELSE 'stale' END AS sync_status ORDER BY sync_quality DESC"
---
...
> **Information-theoretic view:** a WAL delta stream is *residual coding* — the materialized state at the receiver is the predictor, and only the **surprise** (the bits a reader couldn't predict) crosses the wire, so the stream tends toward the entropy rate of the world's change process. This is the "store/transmit only the surprise" principle the engine also measures directly — see [the Information Layer](/docs/concepts/information-layer).
...
Two instances on the same machine or LAN can sync directly over plain HTTP — no cloud round-trip. One instance runs a receive-server over its `ConcurrentStore`; the other pushes and pulls WAL deltas against it. Two routes:

| Route | Body / query | Effect |
|---|---|---|
| `POST /api/sync/push` | a `SyncPayload` JSON | applies the incoming WAL deltas (`sync_import`) |
| `GET /api/sync/pull?since=N` | `since` = last-seen sequence | returns the WAL since `N` as a `SyncPayload` |

The client side is the **`SyncTransport`** contract — `push(payload) -> SyncAck` and `pull(peer_id, since) -> SyncPullResponse`, over the payload types `SyncPayload` / `SyncAck` / `SyncPullResponse`. Two implementations ship: `MemoryTransport` (in-process, for tests and loopback) and `OzWorldTransport` (HTTP to the hosted world). Pointing `OzWorldTransport` at a peer's `serve_sync` address is all a LAN pairing needs.

> **Note:** the swarm/mesh path uses [LAN Discovery](#lan-discovery) (UDP multicast) + WebSocket delta-push, not a separate HTTP transport type. The HTTP `serve_sync` server above is the direct point-to-point LAN lane.

## Supported transports

| Transport | How |
|---|---|
| HTTP push/pull | `arcflow sync push/pull`; the `SyncTransport` contract (`push`/`pull`) with `MemoryTransport` / `OzWorldTransport` impls |
| LAN sync server | `serve_sync` over `ConcurrentStore` — `POST /api/sync/push`, `GET /api/sync/pull?since=N` |
...
See [Sync Architecture](/architecture/sync) for the full protocol specification, implementation waves, and engine primitive mapping.

## See Also

- [Sync Architecture](/architecture/sync) — full protocol specification, vector clocks, and engine primitives
- [Event Sourcing](/event-sourcing) — WAL replay and temporal audit trail built on the same mutation log
- [Live Queries](/live-queries) — live views that sync across nodes as CDC
- [Cloud Architecture](/cloud-architecture) — distributed mesh, ArcFlow WAL Stream, multi-region
