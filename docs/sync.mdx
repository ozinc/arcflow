...
ArcFlow runs locally — in-process, in-browser, at the edge — and the world model replicates to cloud automatically. Local operations are instant (no round-trip, no network dependency). Sync is eventual, conflict-resolved, and resumable: the local world model stays authoritative, cloud stays consistent.
