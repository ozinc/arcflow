...
- [Vector Search](/vector-search) — HNSW k-NN, hybrid search, and embedding management
- [Live Queries](/live-queries) — `LIVE CALL` for incremental algorithm maintenance as standing queries
- [GPU Acceleration](/gpu) — automatic CUDA/Metal dispatch, CSR format, multi-GPU routing
- [Spatial Queries](/worldcypher/spatial) — `algo.nearestNodes()`, frustum queries, R*-tree indexes
