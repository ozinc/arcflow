...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot) WITH s, d.confidence AS confidence, r, s.reliability * d.confidence AS composite_trust RETURN s.name AS sensor, s.reliability AS sensor_quality, confidence, composite_trust, r.name AS entity, r.task, CASE WHEN composite_trust < 0.75 THEN 'E_TRUST_LOW - below minimum threshold' WHEN s.reliability < 0.85 THEN 'W_SENSOR_DEGRADED - sensor reliability nominal' WHEN confidence < 0.80 THEN 'W_DETECTION_WEAK - confidence marginal' ELSE 'OK - observation accepted' END AS error_code ORDER BY composite_trust ASC"
---
