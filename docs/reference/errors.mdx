...
since: "0.1.0"
status: "stable"
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS hazard, count(r) AS entity_count, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, hazard, entity_count, avg_battery, composite_trust, CASE WHEN composite_trust < 0.75 THEN 'E_TRUST_LOW' WHEN s.reliability < 0.85 THEN 'W_SENSOR_DEGRADED' WHEN confidence < 0.80 THEN 'W_DETECTION_WEAK' ELSE 'OK' END AS error_code ORDER BY composite_trust DESC"
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, count(r) AS entities, avg(d.confidence) AS avg_confidence, avg(s.reliability) AS avg_reliability, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, entities, avg_confidence, avg_reliability, composite_trust, CASE WHEN composite_trust < 0.80 THEN 'E_TRUST_LOW' WHEN avg_reliability < 0.90 THEN 'W_SENSOR_DEGRADED' WHEN avg_confidence < 0.85 THEN 'W_DETECTION_WEAK' ELSE 'OK' END AS error_code ORDER BY composite_trust ASC"
...
| `TEMPORAL_WAL_NOT_WIRED` | integration | `AS OF seq N` without WAL context | Use `JournaledStore` or `ConcurrentStore` |
