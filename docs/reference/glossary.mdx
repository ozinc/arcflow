...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH s, d.confidence AS confidence, d.method AS method, r, z, s.reliability * d.confidence AS composite_trust RETURN s.name AS sensor, s.reliability AS sensor_reliability, confidence, method, composite_trust AS composite_trust_definition, r.name AS entity, r.task, r.x AS world_x, r.y AS world_y, z.name AS zone, z.hazard AS hazard_level ORDER BY composite_trust DESC"
---
