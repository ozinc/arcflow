...
**ArcFlow.** A named, versioned, graph-native program that processes nodes with a specific label and derives confidence-scored relationships. Defined with `CREATE SKILL`, triggered with `PROCESS NODE`. Skills implement extraction pipelines, enrichment steps, and relationship derivation as first-class graph objects — each edge they produce carries provenance, a confidence score, and the version that built it.
...
- [Observations & Evidence](/concepts/observations) — ArcFlow Evidence Model in depth
- [Confidence & Provenance](/concepts/confidence-and-provenance) — scoring and tracing facts
- [Temporal](/temporal) — ArcFlow Clock Domains and AS OF queries
- [GPU Acceleration](/gpu) — ArcFlow Graph Kernel, Adaptive Dispatch, and GPU Index in depth
- [Sync](/sync) — ArcFlow WAL Stream and SWMR replication
