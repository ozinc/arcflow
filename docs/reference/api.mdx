...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH s, d.confidence AS confidence, d.method AS method, r, z, s.reliability * d.confidence AS composite_trust RETURN r.name AS robot, r.task, r.battery, r.x AS world_x, r.y AS world_y, confidence, method, composite_trust, s.name AS sensor, s.type AS sensor_modality, s.reliability AS sensor_reliability, z.name AS zone, z.hazard AS zone_risk ORDER BY composite_trust DESC"
---
