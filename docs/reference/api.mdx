...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS hazard, count(r) AS robots, count(s) AS sensors, avg(r.battery) AS avg_battery, max(r.battery) AS max_battery, min(r.battery) AS min_battery, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, hazard, robots, sensors, avg_battery, max_battery, min_battery, composite_trust ORDER BY composite_trust DESC"
