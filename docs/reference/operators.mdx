...
since: "0.26.0"
status: "stable"
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WHERE r.battery >= 60 AND r.speed > 0.5 AND d.confidence > 0.85 AND s.reliability >= 0.88 WITH z.name AS zone, z.hazard AS hazard, count(r) AS entity_count, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, hazard, entity_count, avg_battery, composite_trust, CASE WHEN composite_trust >= 0.90 THEN 'optimal' WHEN composite_trust >= 0.75 THEN 'nominal' ELSE 'degraded' END AS status ORDER BY composite_trust DESC"
playground:
  seed: world
  query: "MATCH (r:Robot)-[:OCCUPIES]->(z:Zone) WHERE r.battery >= 60 AND r.speed > 0.5 AND NOT z.hazard > 0.15 AND (r.task = 'assembly' OR r.task = 'inspection') RETURN r.name AS robot, r.task AS task, r.battery AS battery, r.speed AS speed, z.name AS zone, z.hazard AS hazard ORDER BY r.speed DESC"
