...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WHERE r.battery >= 60 AND r.speed > 0.5 AND d.confidence > 0.85 AND s.reliability >= 0.88 WITH r, s, d, z, s.reliability * d.confidence AS composite_trust, (r.x - s.x) * (r.x - s.x) + (r.y - s.y) * (r.y - s.y) AS range_sq RETURN r.name AS robot, r.task, r.battery, r.speed, composite_trust, range_sq, z.name AS zone, z.hazard ORDER BY composite_trust DESC"
---
