...
playground:
  seed: world
  query: |-
    MATCH (r:Robot) SET r.tier = CASE WHEN r.battery >= 85 THEN 'optimal' WHEN r.battery >= 65 THEN 'nominal' ELSE 'low-power' END

    MATCH (r:Robot)-[:OCCUPIES]->(z:Zone) RETURN r.name AS robot, r.task, r.battery, r.tier, r.x AS world_x, r.y AS world_y, z.name AS zone, z.hazard ORDER BY r.battery DESC
