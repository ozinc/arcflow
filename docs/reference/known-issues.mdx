...
playground:
  seed: world
  query: "MATCH (r:Robot) SET r.status = CASE WHEN r.battery >= 80 THEN 'active' WHEN r.battery >= 60 THEN 'nominal' ELSE 'low-power' END MATCH (r:Robot)-[:OCCUPIES]->(z:Zone) RETURN r.name AS robot, r.task AS task, r.battery AS battery, r.status AS status, r.x AS pos_x, r.y AS pos_y, z.name AS zone, z.hazard AS hazard ORDER BY r.battery DESC"
