...
arcflow-core/Cargo.toml
  [workspace.package]
    version = "0.7.1"            ← The single source of truth
        │
...
|---|---|---|
| `arcflow --version` | `ArcFlow 0.7.1` | `env!("CARGO_PKG_VERSION")` |
| `arcflow status --json` `version` | `"0.7.1"` | same |
| `arcflow doctor` `Engine:` line | `ArcFlow 0.7.1` | same |
| `SHOW SERVER_VERSION` (psql) | `0.7.1` | same |
| `CALL db.version` (WorldCypher) | `"0.7.1"` | same |
| `ArcFlow.version()` (Python) | `"0.7.1"` | FFI symbol — `CARGO_PKG_VERSION` |
| `arcflow.__version__` (Python) | `"0.7.1"` | `importlib.metadata.version("oz-arcflow")` |
| `release-matrix.json` `engine.version` | `"0.7.1"` | `git describe --tags --abbrev=0` |
...
ARCFLOW_VERSION=0.7.1 curl -fsSL https://oz.com/install/arcflow | sh
