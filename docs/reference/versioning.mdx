...
arcflow-core/Cargo.toml
  [workspace.package]
    version = "0.7.2"            ← The single source of truth
        │
...
|---|---|---|
| `arcflow --version` | `ArcFlow 0.7.2` | `env!("CARGO_PKG_VERSION")` |
| `arcflow status --json` `version` | `"0.7.2"` | same |
| `arcflow doctor` `Engine:` line | `ArcFlow 0.7.2` | same |
| `SHOW SERVER_VERSION` (psql) | `0.7.2` | same |
| `CALL db.version` (WorldCypher) | `"0.7.2"` | same |
| `ArcFlow.version()` (Python) | `"0.7.2"` | FFI symbol — `CARGO_PKG_VERSION` |
| `arcflow.__version__` (Python) | `"0.7.2"` | `importlib.metadata.version("oz-arcflow")` |
| `release-matrix.json` `engine.version` | `"0.7.2"` | `git describe --tags --abbrev=0` |
...
- **Patch (`0.7.1 → 0.7.2`)** — bug fix where the documented behavior was wrong. No surface change.
- **Minor (`0.7.x → 0.8.0`)** — additive surface in any layer (new `CALL` procedure, new algorithm, new RPC method, new release artifact). Existing surface unchanged.
...
ARCFLOW_VERSION=0.7.2 curl -fsSL https://oz.com/install/arcflow | sh
