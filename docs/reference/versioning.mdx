...
- **Patch (`0.8.1 → 0.8.2`)** — bug fix where the documented behavior was wrong. No surface change.
- **Minor (`0.8.x → 0.9.0`)** — additive surface in any layer (new `CALL` procedure, new algorithm, new RPC method, new release artifact). Existing surface unchanged.
...
4. **`0.x` is alpha.** `1.0` is reserved for the first
