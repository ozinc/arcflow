---
title: "Platforms"
description: "Native graph engine — scales from a single process to an autonomous mesh"
section: "platform"
order: 1
since: "10.0.0"
status: "stable"
...
ArcFlow is a native graph engine. It runs at full speed on your machine, auto-detects GPU hardware, and scales from a single process to an autonomous mesh of nodes. Every deployment (desktop, server, mobile, edge) runs the same engine with the same query language and the same algorithms.
...
| **Serving** | Add `--serve 7687` for TCP, `--http 8080` for REST, `arcflow-mcp` for AI agents |
...
[Open ArcFlow Engine](/engine) in your browser. No installation. Persistent storage. Works offline. Syncs to [ArcFlow Cloud](/arcflow/cloud) when you're ready.
...
| **20+ algorithms** | Full | Full | Full | Full | Full |
