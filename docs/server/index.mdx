...
arcflow-mcp --data-dir ./graph       # MCP server for AI agents
```
...
The MCP (Model Context Protocol) server provides structured tool access for AI agents over stdio JSON-RPC. This is the fallback for environments without filesystem access (cloud IDEs, sandboxed agents).

For environments with filesystem access, prefer `arcflow query "..." --json` instead — it is simpler and faster.
