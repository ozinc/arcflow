---
title: "Server Modes & PG Wire"
kind: operation
description: "Access the operational world model over PostgreSQL wire protocol, HTTP REST, TCP, or MCP — same engine, same WorldCypher queries, same world model everywhere"
status: "stable"
playground:
  seed: world
...
- **Extended Query protocol**: Parse/Bind/Describe/Execute/Sync/Close (parameter placeholders `$1`/`$2` are parsed but not yet substituted — use the Simple Query protocol with inline values or the SDK's `params` argument instead)
...
Response includes: 29 graph algorithms (pageRank, louvain, dijkstra, graphRAG, etc.), 100+ procedures, 93 built-in functions, observation classes (observed, inferred, predicted), and all supported query clauses.
...
- [Persistence](/server/persistence) — WAL-backed durable storage
