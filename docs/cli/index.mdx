...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH s, d.method AS method, d.confidence AS confidence, r, z, s.reliability * d.confidence AS composite_trust RETURN s.name AS sensor, s.type AS type, s.reliability AS reliability, method, confidence, composite_trust, r.name AS robot, r.task AS task, r.battery AS battery, z.name AS zone, z.hazard AS hazard ORDER BY composite_trust DESC"
---
