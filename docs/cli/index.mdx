...
title: "CLI"
kind: operation
description: "arcflow — world model intelligence for the command line. Like grep, find, and jq, but for operational world models."
status: "stable"
playground:
  seed: world
...
Every CLI result envelope carries a `__snapshot` field — the URI of the snapshot the
query observed:
...
See [Snapshot-Pinned Reads](/concepts/snapshots) for the full provenance story.
...
Two projections of the same snapshot are byte-identical. See
[Filesystem Projection](/cli/project) for the agent-grep workflow.
...
## Upgrading — `arcflow upgrade`

Keep the CLI current without re-running the install script:

```bash
arcflow upgrade           # fetch the latest release and replace this binary in place
arcflow upgrade --check   # report whether a newer version exists; change nothing
```

`upgrade` resolves the latest version from `ozinc/arcflow` GitHub Releases (the same source as the [install script](/docs/installation)), compares it to the running binary, and self-replaces only if it's strictly newer. Set `ARCFLOW_REPO=<owner>/<repo>` to upgrade from a mirror or fork instead.
...
- [Filesystem Projection](/cli/project) — `arcflow project` writes a snapshot as a directory tree
- [Plugin Management](/cli/plugin) — `arcflow plugin install/uninstall/list/verify`
- [Agent Governance](/cli/governance) — receipts, hooks, and the agent verification state machine

## See Also

- [Agent-Native Database](/agent-native) — why CLI-first beats MCP for shell agents; filesystem workspace patterns
- [ArcFlow for Coding Agents](/docs/guides/agent-quickstart) — structured errors, batch execution, checkpointing
- [Installation](/installation) — binary, npm, pip, cargo, and WASM install paths
- [Snapshot-Pinned Reads](/concepts/snapshots) — `--at-snapshot`, `arcflow rerun`, and `__snapshot` provenance
