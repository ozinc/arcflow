...
Vector throughput depends on host CPU/GPU and embedding dimension.
Measure on the hardware you'll deploy on:
...
GPU acceleration is automatic based on index size — no configuration
required. On CUDA hardware with larger vector collections,
**ArcFlow Adaptive Dispatch** routes automatically to the
GPU-accelerated nearest-neighbour path. Same query, zero configuration.
