...
On CUDA hardware with larger vector collections, ArcFlow automatically routes to its GPU-accelerated nearest neighbor backend — a GPU-resident graph structure that achieves higher throughput than CPU HNSW at scale. Same query, automatic routing.
