...
This architecture eliminates entire categories of infrastructure: no external cache service (the graph is already in-process), no separate vector database (HNSW indexes live alongside graph data), no workflow engine for orchestration (procedures run in the same runtime). The result is fewer moving parts, lower latency, and a single binary to deploy.
