...
- [Threading Model](/docs/concepts/threading-model) — the many-reader / one-writer-per-handle concurrency contract; ArcSwap snapshots for lock-free reads; the `HANDLE_BUSY_CONCURRENT_WRITER` typed-error guard.
- [World Graph Substrate (Preview)](/docs/architecture/worldgraph) — engine-architecture preview of the substrate the World Graph layer is being built on (module structure, virtual labels, residency tiers, `oz://` URIs, ARC1 format)
