...
description: "The grounding layer between neural world models and the operational world model — lift Cosmos/Genie detections into persistent, queryable entities with spatial position, confidence, and temporal history."
section: "use-cases"
status: "stable"
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS hazard, count(r) AS entity_count, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, hazard, entity_count, avg_battery, composite_trust, CASE WHEN composite_trust >= 0.92 THEN 'grounded' WHEN composite_trust >= 0.80 THEN 'partially-grounded' ELSE 'ungrounded' END AS grounding_status ORDER BY composite_trust DESC"

---

# Use Case: Grounded Neural Objects

Neural world models (NVIDIA Cosmos, DeepMind Genie, Tesla FSD) generate detections, pose estimates, predicted future states. Their output is ephemeral — tensors, tracks, video latents. They do not remember. They do not version. They cannot tell you where a specific entity was at a specific timestamp with a specific confidence score from a specific sensor.
...
The query no neural world model can answer:
...
Deterministic. Sub-millisecond. Against the exact history of every detection, every sensor, every identity assignment. This is operational causality — the actual causal chain from sensor to entity to zone to consequence, stored as a graph, traversable instantly. No neural world model can answer it. ArcFlow answers it by construction.

Neural world models simulate. ArcFlow records. The GNO is the point where both layers meet.

## See Also

- [Robotics & Perception](/use-cases/robotics) — sensor fusion, track lifecycle, confidence-scored observations
- [Trusted RAG](/trusted-rag) — confidence-filtered retrieval for learned properties
- [Confidence & Provenance](/concepts/confidence-and-provenance) — observation classes and evidence chains
- [Live Queries](/live-queries) — standing queries that update learned properties as new evidence arrives
- [Autonomous Systems](/use-cases/autonomous-systems) — fleet-level coordination on a shared world model
