...
section: "use-cases"
status: "stable"
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS hazard, count(r) AS entity_count, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, hazard, entity_count, avg_battery, composite_trust, CASE WHEN grounding_confidence >= 0.90 THEN 'grounded' WHEN grounding_confidence >= 0.80 THEN 'partially grounded' ELSE 'ungrounded' END AS composite_trust ORDER BY composite_trust DESC"
playground:
  seed: world
...
Neural world models predict. ArcFlow remembers. The GNO is the point where both layers meet.
