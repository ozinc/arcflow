...
section: "use-cases"
status: "stable"
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS hazard, count(r) AS entity_count, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, hazard, entity_count, avg_battery, composite_trust, CASE WHEN composite_trust >= 0.9 AND r.battery >= 80 THEN 'autonomous' WHEN composite_trust >= 0.8 THEN 'supervised' ELSE 'degraded' END AS operational_mode ORDER BY composite_trust DESC"
playground:
  seed: world
