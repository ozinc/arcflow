...
playground:
  ...
  query: |-
    ...
    MERGE (d:Agent {id: 'director'})-[:DELEGATES_TO {intent: 'monitor', priority: 1, confidence_floor: 0.85}]->(p1:Agent {id: 'plan-a'})

    MERGE (d:Agent {id: 'director'})-[:DELEGATES_TO {intent: 'inspect', priority: 2, confidence_floor: 0.80}]->(p2:Agent {id: 'plan-b'})

    MERGE (p1:Agent {id: 'plan-a'})-[:MONITORS {freq_hz: 25}]->(s1:Sensor {id: 'cam-1'})

    MERGE (p2:Agent {id: 'plan-b'})-[:MONITORS {freq_hz: 10}]->(s2:Sensor {id: 'lidar-1'})
