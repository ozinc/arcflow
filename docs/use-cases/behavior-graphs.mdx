...
This matters for autonomous agents and game AI because behavior is inseparable from world state. An agent that can traverse the same graph it lives in — seeing its own history, querying its environment, following relationships to nearby entities — makes decisions grounded in the full world model rather than a shallow key-value blackboard.
