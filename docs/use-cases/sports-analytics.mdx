...
- [Spatial Queries](/spatial-knowledge) — full spatial primitives reference
- [Programs](/programs) — bundle YOLO detectors, player trackers, and PTZ controllers into installable manifests with hardware validation
- [Triggers](/worldcypher/triggers) — fire a skill automatically when a new `:Frame` or `:Event` node enters the graph
- [Live Queries](/live-queries) — standing queries for real-time formation and zone tracking
- [Multi-Stream Spatiotemporal World Model](/arcflow/cookbooks/multi-stream-spatiotemporal-world-model) — runnable, CI-tested cookbook (Python) covering the multi-camera spatial + temporal pattern
