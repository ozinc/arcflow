...
playground:
  query: |-
    ...
    MATCH (s1:Sensor {id: 'temp-01'}) MATCH (t1:Asset {id: 'turbine-01'}) MERGE (s1)-[:MONITORS {confidence: 0.97, freq_hz: 10}]->(t1)
    MATCH (s1:Sensor {id: 'temp-01'}) MATCH (t2:Asset {id: 'turbine-02'}) MERGE (s1)-[:MONITORS {confidence: 0.94, freq_hz: 10}]->(t2)
    MATCH (s2:Sensor {id: 'vib-01'}) MATCH (t2:Asset {id: 'turbine-02'}) MERGE (s2)-[:MONITORS {confidence: 0.91, freq_hz: 100}]->(t2)
    MATCH (s3:Sensor {id: 'press-01'}) MATCH (p1:Asset {id: 'pump-01'}) MERGE (s3)-[:MONITORS {confidence: 0.96, freq_hz: 5}]->(p1)
