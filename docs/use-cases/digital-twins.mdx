...
section: "use-cases"
status: "stable"
  query: "MERGE (t1:Asset {id: 'turbine-01', name: 'Turbine-01', type: 'turbine', rpm: 1450.0, temp: 72.3, vibration: 0.02, status: 'nominal'}) MERGE (t2:Asset {id: 'turbine-02', name: 'Turbine-02', type: 'turbine', rpm: 1510.0, temp: 88.7, vibration: 0.09, status: 'warning'}) MERGE (p1:Asset {id: 'pump-01', name: 'Pump-01', type: 'pump', pressure: 4.2, flow: 85.0, temp: 42.1, status: 'nominal'}) MERGE (s1:Sensor {id: 'temp-01', name: 'TempSensor-N1', type: 'thermal', reliability: 0.98}) MERGE (s2:Sensor {id: 'vib-01', name: 'VibSensor-01', type: 'accelerometer', reliability: 0.95}) MERGE (s3:Sensor {id: 'press-01', name: 'PressureSensor-01', type: 'pressure', reliability: 0.97}) MERGE (s1:Sensor {id: 'temp-01'})-[:MONITORS {confidence: 0.97, freq_hz: 10}]->(t1:Asset {id: 'turbine-01'}) MERGE (s1:Sensor {id: 'temp-01'})-[:MONITORS {confidence: 0.94, freq_hz: 10}]->(t2:Asset {id: 'turbine-02'}) MERGE (s2:Sensor {id: 'vib-01'})-[:MONITORS {confidence: 0.91, freq_hz: 100}]->(t2:Asset {id: 'turbine-02'}) MERGE (s3:Sensor {id: 'press-01'})-[:MONITORS {confidence: 0.96, freq_hz: 5}]->(p1:Asset {id: 'pump-01'}) MATCH (sensor:Sensor)-[m:MONITORS]->(asset:Asset)  RETURN sensor.name AS sensor, sensor.type, sensor.reliability, confidence, composite_trust, asset.name AS asset, asset.type, asset.status, CASE WHEN asset.status = 'warning' AND composite_trust >= 0.88 THEN 'ALERT - confirmed, inspect now' WHEN asset.status = 'warning' THEN 'WARN - low-trust reading, verify first' ELSE 'nominal' END AS action ORDER BY composite_trust DESC"
playground:
  query: "MERGE (t1:Asset {id: 'turbine-01', name: 'Turbine-01', type: 'turbine', rpm: 1450.0, temp: 72.3, vibration: 0.02, status: 'nominal', trust: 0.97})\n\nMERGE (t2:Asset {id: 'turbine-02', name: 'Turbine-02', type: 'turbine', rpm: 1510.0, temp: 88.7, vibration: 0.09, status: 'warning', trust: 0.91})\n\nMERGE (s1:Sensor {id: 'temp-01', name: 'TempSensor', type: 'thermal', reliability: 0.98})\n\nMERGE (s2:Sensor {id: 'vib-01', name: 'VibSensor', type: 'accelerometer', reliability: 0.95})\n\nMERGE (s1:Sensor {id: 'temp-01'})-[:MONITORS]->(t1:Asset {id: 'turbine-01'})\n\nMERGE (s2:Sensor {id: 'vib-01'})-[:MONITORS]->(t2:Asset {id: 'turbine-02'})\n\nMATCH (sensor:Sensor)-[:MONITORS]->(asset:Asset) RETURN sensor.name AS sensor, sensor.type AS sensor_type, sensor.reliability AS sensor_reliability, asset.name AS asset, asset.type AS asset_type, asset.status AS status, asset.temp AS temperature, asset.vibration AS vibration, asset.trust AS trust_score, CASE WHEN asset.status = 'warning' AND sensor.reliability >= 0.95 THEN 'alert-confirmed' WHEN asset.status = 'warning' THEN 'alert-low-confidence' ELSE 'nominal' END AS action ORDER BY sensor.reliability DESC"
