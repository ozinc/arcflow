...
title: "Agent Tooling"
kind: use-case
description: "ArcFlow as ephemeral in-process world model for coding agents — spin up a spatial-temporal graph, run 29 algorithms, query results, and discard. No server, no setup."
status: "stable"
playground:
  seed: world
...
The browser runtime at [/engine](/engine) gives agents another tool: verify a query works by running it in the playground before writing it into code.
...
```bash
# Symbol lookup — single shell exec, no protocol handshake
...
- [ArcFlow for Coding Agents](/docs/guides/agent-quickstart) — CLI patterns, structured errors, batch execution
