...
Claude Code, Codex CLI, Gemini CLI, and similar tools already have a shell tool. Give them the `arcflow` binary and they call it exactly like `grep` or `git` — no protocol, no handshake, no session:
...
MCP is the right integration when the agent runs in a browser or cloud sandbox with no local shell — ChatGPT, Claude.ai, Gemini web. These interfaces cannot run `arcflow` as a binary, so MCP is the only way to give them graph tool access:

```json
// Claude.ai / ChatGPT plugin configuration — NOT for Claude Code
{
  "mcpServers": {
    "arcflow": {
      "command": "npx",
...
| MCP server | Cloud chat UIs (ChatGPT, Claude.ai) that have no local shell |
