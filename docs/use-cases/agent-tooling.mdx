...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WHERE d.confidence > 0.85 WITH s, d, r, z, d.confidence * s.reliability AS composite_trust RETURN r.name AS entity, r.task, r.x AS pos_x, r.y AS pos_y, r.battery, composite_trust, d.method AS perception_source, z.name AS zone, z.hazard, CASE WHEN composite_trust >= 0.90 AND z.hazard > 0.15 THEN 'ALERT - high-risk zone, trusted observation' WHEN composite_trust >= 0.85 THEN 'TRACK - nominal operation' ELSE 'IGNORE - insufficient trust' END AS agent_decision ORDER BY composite_trust DESC"
---
