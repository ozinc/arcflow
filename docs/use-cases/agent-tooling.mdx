...
section: "use-cases"
status: "stable"
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WHERE d.confidence > 0.85 WITH z.name AS zone, z.hazard AS hazard, count(r) AS entity_count, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, hazard, entity_count, avg_battery, composite_trust, CASE WHEN composite_trust >= 0.90 AND z.hazard > 0.15 THEN 'ALERT' WHEN composite_trust >= 0.85 THEN 'TRACK' ELSE 'IGNORE' END AS agent_decision ORDER BY composite_trust DESC"
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WHERE d.confidence > 0.85 WITH z.name AS zone, z.hazard AS hazard, count(r) AS active_entities, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS observation_quality RETURN zone, hazard, active_entities, avg_battery, observation_quality, CASE WHEN observation_quality >= 0.90 AND z.hazard > 0.15 THEN 'alert' WHEN observation_quality >= 0.85 THEN 'track' ELSE 'ignore' END AS agent_action ORDER BY observation_quality DESC"
