...
status: "stable"
playground:
  query: "MERGE (a1:Account {id: 'acc-001', name: 'Legit Corp', balance: 125000.0, kyc: 1, flagged: 0, sent: 48000.0}) MERGE (a2:Account {id: 'acc-002', name: 'Shell Alpha', balance: 840.0, kyc: 0, flagged: 1, sent: 47500.0}) MERGE (a3:Account {id: 'acc-003', name: 'Shell Beta', balance: 210.0, kyc: 0, flagged: 1, sent: 47200.0}) MERGE (a4:Account {id: 'acc-004', name: 'Offshore Sink', balance: 0.0, kyc: 0, flagged: 1, sent: 0.0}) MERGE (a1x:Account {id: 'acc-001'})-[:TRANSFERRED_TO]->(a2x:Account {id: 'acc-002'}) MERGE (a2y:Account {id: 'acc-002'})-[:TRANSFERRED_TO]->(a3y:Account {id: 'acc-003'}) MERGE (a3z:Account {id: 'acc-003'})-[:TRANSFERRED_TO]->(a4z:Account {id: 'acc-004'}) MATCH (src:Account)-[:TRANSFERRED_TO]->(mid:Account)-[:TRANSFERRED_TO]->(sink:Account) WHERE mid.flagged = 1 AND sink.flagged = 1 RETURN src.name AS origin, src.balance AS source_balance, src.kyc AS kyc_verified, src.sent AS outgoing_amount, mid.name AS intermediary, mid.kyc AS mid_kyc, mid.sent AS forwarded_amount, sink.name AS destination, sink.flagged AS sink_flagged, src.sent - mid.sent AS value_extracted ORDER BY value_extracted DESC"
