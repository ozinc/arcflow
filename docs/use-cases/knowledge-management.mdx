...
status: "stable"
playground:
  query: "MERGE (auth1:Author {id: 'auth-chen', name: 'Dr. Chen', org: 'OZ Labs', reputation: 0.94}) MERGE (auth2:Author {id: 'auth-park', name: 'Dr. Park', org: 'MIT', reputation: 0.91}) MERGE (d1:Document {id: 'doc-wm', title: 'World Models at Scale', category: 'research', relevance: 0.96, citations: 142}) MERGE (d2:Document {id: 'doc-sp', title: 'Spatial Graph Indexing', category: 'systems', relevance: 0.88, citations: 89}) MERGE (d3:Document {id: 'doc-cw', title: 'Confidence-Weighted Retrieval', category: 'ai', relevance: 0.83, citations: 47}) MERGE (auth1:Author {id: 'auth-chen'})-[:AUTHORED]->(d1:Document {id: 'doc-wm'}) MERGE (auth2:Author {id: 'auth-park'})-[:AUTHORED]->(d2:Document {id: 'doc-sp'}) MERGE (auth2:Author {id: 'auth-park'})-[:AUTHORED]->(d3:Document {id: 'doc-cw'}) MERGE (d1x:Document {id: 'doc-wm'})-[:CITES]->(d2x:Document {id: 'doc-sp'}) MERGE (d1y:Document {id: 'doc-wm'})-[:CITES]->(d3y:Document {id: 'doc-cw'}) MATCH (auth:Author)-[:AUTHORED]->(doc:Document)-[:CITES]->(ref:Document) RETURN auth.name AS author, auth.org AS org, auth.reputation AS reputation, doc.title AS document, doc.category AS category, doc.relevance AS relevance, doc.citations AS citations, ref.title AS cites_document, ref.citations AS cited_citations ORDER BY doc.relevance DESC"
