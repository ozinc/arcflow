...
playground:
  query: |-
    ...
    MATCH (auth1:Author {id: 'auth-wm'}) MATCH (d1:Document {id: 'doc-wm'}) MERGE (auth1)-[:AUTHORED {peer_reviewed: 1}]->(d1)
    MATCH (auth2:Author {id: 'auth-sp'}) MATCH (d2:Document {id: 'doc-sp'}) MERGE (auth2)-[:AUTHORED {peer_reviewed: 1}]->(d2)
    MATCH (auth2:Author {id: 'auth-sp'}) MATCH (d3:Document {id: 'doc-conf'}) MERGE (auth2)-[:AUTHORED {peer_reviewed: 0}]->(d3)
    MATCH (d1x:Document {id: 'doc-wm'}) MATCH (d2x:Document {id: 'doc-sp'}) MERGE (d1x)-[:CITES {strength: 0.88, essential: 1}]->(d2x)
    MATCH (d1x:Document {id: 'doc-wm'}) MATCH (d3x:Document {id: 'doc-conf'}) MERGE (d1x)-[:CITES {strength: 0.74, essential: 0}]->(d3x)
