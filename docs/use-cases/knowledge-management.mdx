...
playground:
  query: |-
    ...
    MATCH (auth1:Author {id: 'auth-wm'}), (d1:Document {id: 'doc-wm'}) MERGE (auth1)-[:AUTHORED {peer_reviewed: true}]->(d1)
    MATCH (auth2:Author {id: 'auth-sp'}), (d2:Document {id: 'doc-sp'}) MERGE (auth2)-[:AUTHORED {peer_reviewed: true}]->(d2)
    MATCH (auth2:Author {id: 'auth-sp'}), (d3:Document {id: 'doc-conf'}) MERGE (auth2)-[:AUTHORED {peer_reviewed: false}]->(d3)
    MATCH (d1x:Document {id: 'doc-wm'}), (d2x:Document {id: 'doc-sp'}) MERGE (d1x)-[:CITES {strength: 0.88, essential: true}]->(d2x)
    MATCH (d1x:Document {id: 'doc-wm'}), (d3x:Document {id: 'doc-conf'}) MERGE (d1x)-[:CITES {strength: 0.74, essential: false}]->(d3x)
