...
playground:
  query: |-
    ...
    MERGE (auth1:Author {id: 'auth-wm'})-[:AUTHORED {peer_reviewed: 1}]->(d1:Document {id: 'doc-wm'})

    MERGE (auth2:Author {id: 'auth-sp'})-[:AUTHORED {peer_reviewed: 1}]->(d2:Document {id: 'doc-sp'})

    MERGE (auth2:Author {id: 'auth-sp'})-[:AUTHORED {peer_reviewed: 0}]->(d3:Document {id: 'doc-conf'})

    MERGE (d1x:Document {id: 'doc-wm'})-[:CITES {strength: 0.88, essential: 1}]->(d2x:Document {id: 'doc-sp'})

    MERGE (d1x:Document {id: 'doc-wm'})-[:CITES {strength: 0.74, essential: 0}]->(d3x:Document {id: 'doc-conf'})
