...
section: "use-cases"
status: "stable"
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS hazard, count(r) AS entity_count, avg(r.battery) AS avg_battery, avg(s.reliability * d.confidence) AS composite_trust RETURN zone, hazard, entity_count, avg_battery, composite_trust, CASE WHEN r.speed > 1.0 AND composite_trust > 0.88 THEN 'track' WHEN r.speed > 0.5 THEN 'observe' ELSE 'hold' END AS robot_action ORDER BY composite_trust DESC"
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH z.name AS zone, z.hazard AS hazard, count(r) AS detected_robots, avg(r.battery) AS avg_battery, avg(r.speed) AS avg_speed, avg(s.reliability * d.confidence) AS perception_quality RETURN zone, hazard, detected_robots, avg_battery, avg_speed, perception_quality, CASE WHEN perception_quality >= 0.90 AND avg_speed > 0.5 THEN 'full-autonomy' WHEN perception_quality >= 0.80 THEN 'supervised' ELSE 'manual-override' END AS control_mode ORDER BY perception_quality DESC"
