...
playground:
  ...
  query: |-
    ...
    WITH s, d, r, z, s.reliability * d.confidence AS composite_trust, (r.x - s.x) * (r.x - s.x) + (r.y - s.y) * (r.y - s.y) AS dist_sq
    RETURN s.name AS sensor, s.type, s.x AS sensor_x, s.y AS sensor_y, s.reliability, d.method, d.confidence, composite_trust, dist_sq AS proximity_sq, r.name AS robot, r.x AS robot_x, r.y AS robot_y, r.task, z.name AS zone ORDER BY composite_trust DESC
