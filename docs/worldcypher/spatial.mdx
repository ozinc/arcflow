...
playground:
  seed: world
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) WITH s, d.method AS method, d.confidence AS confidence, r, z, s.reliability * d.confidence AS composite_trust, (r.x - s.x) * (r.x - s.x) + (r.y - s.y) * (r.y - s.y) AS dist_sq RETURN s.name AS sensor, s.type AS type, s.x AS sensor_x, s.y AS sensor_y, s.reliability AS reliability, method, confidence, composite_trust, dist_sq AS proximity_sq, r.name AS robot, r.x AS robot_x, r.y AS robot_y, r.task AS task, z.name AS zone ORDER BY composite_trust DESC"
---
