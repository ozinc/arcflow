...
|---|---|---|---|---|
| `CALL algo.sensorReliability('sensor-id')` | `accuracy`, `drift_rate`, `sample_count`, `calibration_recommended` | Compute mean confidence across all Entity nodes sourced from this sensor. Updates `reliability_7d` or `reliability_30d` on the Sensor node. Pass `'30d'` as second arg for 30-day window. | 0.7.1 | Yes |
