...
| `CALL db.capabilities()` | `capability`, `value` | Engine version, GPU backend status, compute capabilities, index counts, memory footprint, and algorithm availability | 11.0.0 |
...
| `CALL db.embeddingStats()` | `model`, `version`, `count`, `oldest_embedded_at` | Embedding statistics by model/version including count and oldest embedding timestamp | 11.0.0 |
| `CALL db.explainSpatialJoin(left_label, left_key, right_label, right_key)` | `strategy`, `left_count`, `right_count`, `threshold` | Spatial join planner explanation with chosen strategy and node counts per side | 11.0.0 |
| `CALL db.idFrom(key1, key2, ...)` | `nodeId`, `keys` | Deterministic content-addressed NodeId from key values via FNV-1a hashing | 11.0.0 |
...
| `CALL algo.labelPropagation([label_property], [rel_types])` | `node_id`, `propagated_label`, `label_confidence`, `hops_from_seed` | Stochastic label propagation over optional relation types | 11.0.0 |
| `CALL algo.cAndSLabelPropagation([label_property], [rel_types])` | `node_id`, `propagated_label`, `label_confidence`, `hops_from_seed` | Stochastic label propagation, C&S variant with seed-anchored confidence | 11.0.0 |
...
| `CALL algo.cycleDetectionDirected()` | `hasCycle`, `cycleNodes`, `cycleNodeCount` | Directed cycle detection (Kosaraju SCC; cuGraph on GPU, fallback on CPU) | 11.0.0 |
| `CALL algo.cycleDetectionUndirected()` | `hasCycle`, `cycleNodes`, `cycleNodeCount` | Undirected cycle detection via DFS | 11.0.0 |
...
| `CALL algo.maxFlow(sourceId, sinkId, [capacityProperty])` | `maxFlow`, `source`, `sink` | Maximum flow from source to sink using a capacity edge property (default `capacity`) | 11.0.0 |
...
| `CALL algo.pairSimilarity(sourceIds, targetIds)` | `node1`, `node2`, `score` | Jaccard similarity for explicit node pairs (variadic — accept paired or interleaved args) | 11.0.0 |
...
| `CALL algo.graphRAGMultiModel(label, [k], [queryVector])` | `nodeId`, `fusedScore`, `modelCount` | Multi-model RAG fusing scores across all embedding properties on a label | 11.0.0 |
| `CALL algo.similarThenTraverse(label, embeddingProperty, queryVector, [k], [maxHops], [relType], [minEdgeConfidence])` | `seedEntityId`, `vectorSimilarity`, `reachableJson`, `backend` | k-NN vector seed + per-seed BFS traversal with optional relation-type and confidence filters | 11.0.0 |
...
| Procedure | Return Columns | Description | Since |
|---|---|---|---|
| `CALL algo.node2vec(dims, walkLen, walks)` | _(sets node.embedding)_ | Structural random walk embeddings | 10.0.0 |
...
| `CALL algo.embeddingProperties([label])` | `property`, `dimensions`, `withProvenance` | List embedding properties for a label, with dimensions and provenance count | 11.0.0 |
...
| `CALL algo.confidenceCalibration(entityId, [window])` | `ece_score`, `sample_count`, `calibration_curve`, `calibration_status` | Expected Calibration Error (ECE) — predicted confidence vs observed accuracy (WorldScore methodology) | 11.0.0 |
| `CALL algo.predictionDrift(entityId, [horizonMs])` | `entity_id`, `horizon_ms`, `predicted_x`, `observed_x`, `delta_x`, `predicted_y`, `observed_y`, `delta_y`, `delta_magnitude`, `prediction_source`, `observation_seq`, `predicted_seq` | Drift between predicted and observed entity positions within a time horizon | 11.0.0 |
| `CALL algo.temporalDecay(nodeId, [halfLifeDays], [floor], [staleThreshold])` | `decayed_confidence`, `days_elapsed`, `is_stale`, `freshness_class` | Lazy temporal decay of confidence based on last corroboration timestamp | 11.0.0 |
...
| `CALL db.storageMode()` | `mode`, `node_count`, `csr_warm`, `csr_auto_threshold`, `dense_store_enabled`, `dense_store_nodes` | Active storage mode (HashMap, DenseStore, CSR, or hybrid) with thresholds | 11.0.0 |
| `CALL db.csrStats()` | `status`, `vertices`, `edges` | CSR (Compressed Sparse Row) cache statistics — warm/stale status, vertex / edge counts, pending delta size | 11.0.0 |
| `CALL db.denseStore()` | `enabled`, `nodes`, `tables`, `memory_bytes`, `coverage` | DenseStore status with node and table counts, memory footprint, coverage percentage | 11.0.0 |
| `CALL db.warmCsr()` | `status` | Warm the CSR cache; returns `warm` or `rebuilt` | 11.0.0 |
| `CALL db.parallelConfig()` | `morsel_size`, `rayon_threads`, `dense_store_coverage`, `csr_status` | Morsel scheduler and parallelism configuration including thread count | 11.0.0 |
...
| `CALL db.requireExecutionContext('local_cpu' \| 'local_gpu' \| 'distributed')` | `context`, `status` | Enforces a required execution context; raises a hard error on mismatch | 11.0.0 |
| `CALL db.syncConflicts()` | `id`, `local_op`, `remote_op`, `local_clock`, `remote_clock`, `resolution`, `timestamp` | Replication conflict log with operations, vector clocks, and resolution per conflict | 11.0.0 |
...
| Procedure | Return Columns | Description | Since |
|---|---|---|---|
| `CALL db.gpuStack()` | `cuda`, `thrust`, `cugraph`, `cuvs` | GPU stack availability — CUDA driver, Thrust, cuGraph, cuVS. Optional fields: `compute_capability`, `gpu_device`, `cugraph_algorithms`, `cuvs_algorithms` | 11.0.0 |
| `CALL db.gpuCsrStatus()` | `resident`, `size_bytes`, `mutation_sequence` | GPU CSR cache residency including size in bytes and the mutation sequence the cache reflects | 11.0.0 |
| `CALL db.gpuVectorSearch(query, label, property, k)` | `nodeId`, `similarity`, `backend` | GPU-accelerated vector similarity search via cuVS CAGRA (falls back to CPU HNSW) | 11.0.0 |
| `CALL algo.gpu_capabilities()` | `symbol`, `available`, `detail` | GPU algorithm capability report — cuGraph availability, compute capability, per-algorithm probes | 11.0.0 |
...
| Procedure | Return Columns | Description | Since |
|---|---|---|---|
| `CALL db.otelPolicy()` | `policy` | Current span recording policy (`off`, `lite`, or `full`) | 11.0.0 |
| `CALL db.setOtelPolicy('off' \| 'lite' \| 'full')` | `policy` | Set span recording policy and return the new value | 11.0.0 |
...
| Procedure | Return Columns | Description | Since |
|---|---|---|---|
| `CALL db.extensions()` | `name`, `description`, `syntax`, `reference` | All ArcFlow-unique query language extensions — LIVE views, Evidence algebra, write-back surfaces | 11.0.0 |
