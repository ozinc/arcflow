...
description: "11 aggregation functions + 7 window functions — GROUP BY, LAG, LEAD, ROW_NUMBER, PERCENT_RANK, STDDEV_POP"
section: "worldcypher/functions"
order: 9
since: "0.4.0"
status: "stable"
  query: "MATCH (s:Sensor)-[d:DETECTED]->(r:Robot)-[:OCCUPIES]->(z:Zone) RETURN z.name AS zone, z.hazard, count(r) AS robot_count, avg(r.battery) AS avg_battery, avg(r.speed) AS avg_speed, avg(s.reliability * d.confidence) AS avg_detection_quality ORDER BY avg_detection_quality DESC"
playground:
  seed: world
