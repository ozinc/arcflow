...
- `ARCFLOW_ENGINE_PATH=~/code/arcflow-core scripts/sync-conformance-data.sh` — clean (6 files synced).
- `scripts/generate-reference.py` — patched to drop the now-stale `Engine version` row from `render_dashboard()` (the source field was removed upstream); script then runs clean, 1 top-level page updated.
- `scripts/lint-version-literals.py` — dropped the 3 conformance-file allowlist entries; lint now `OK — no hardcoded ArcFlow version literals outside SoT-bearing files`.
- Schema-sync confirmed unchanged (no `typescript/src/code-intelligence.ts` modifications needed — v0.8.1 didn't touch schema constants).

The DOC-AF-2026-05-14-002 + DOC-AF-2026-05-14-003 threads (the original asks this closure addresses) were already in `resolved/` from May 14; no further DOC-side housekeeping needed on those.
