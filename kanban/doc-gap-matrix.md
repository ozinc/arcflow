...
- **2026-06-24 (iter 44, closing-phase A2)** Arm-G broad vocab sweep across docs/. Removed the back-compat cruft: installation.mdx 'Upgrading from a pre-2026-05-14 install (the v1.x line)' + 'Why this happens' sections (25 lines) — greenfield/pre-launch has no v1.x-binary users; reinstall path already in Quick install. create-node-label.mdx: dropped 'legacy' qualifier (per naming.mdx guidance). Audited the rest: remaining matches are legitimate (template status enums, prose 'no longer', REMOVE example field-names, /healthz current alias, AS-OF error-code fact). Arm-G vocab sweep COMPLETE. NEXT: Arm-E SSoT coherence pass (concepts/ first).
- **2026-06-24 (iter 45, closing-phase Arm-E area 1/6)** Confidence/provenance SSoT: AUDITED — already coherent (no duplicate definitions). evidence-model.mdx is a clean umbrella HUB (3 sub-concepts, links out, defines nothing twice); confidence-and-provenance.mdx is canonical for the _confidence score + edge-provenance props; causal-edges.mdx owns CAUSED_BY; observations.mdx owns classes. Only gap: confidence-and-provenance See Also didn't link UP to the evidence-model hub → added backlink. WATCH ITEM (sweep c, anti-vaporware DEFER): new core feats 838cd446 EvidenceField (arcflow-core/src/evidence_field.rs) + 25894e59 ray_lift — internal substrate for I-INIT-WBF (World Belief Field / collaborative-perception); NOT in PUBLIC-SURFACE.md, not dispatched → not public yet; do not document until shipped+public. Next Arm-E area 2/6: observation classes + planes.
- **2026-06-24 (iter 46, Arm-E area 2/6)** Observation classes + planes SSoT: AUDITED coherent. observations.mdx canonical for observed/inferred/predicted (§Three observation classes); confidence-and-provenance's 'Observation Class + Confidence' is a two-axis INTERACTION table (legit usage, not a redefinition). Added evidence-model hub backlink to observations.mdx See Also (parallel to area 1). PLANES (semantic/scene): no duplicate defs — no concept page defines them; only surfaced via documented db.nodesByPlane.semantic()/.scene() procs (AGENTS.md). Not a dedup issue (optional minor coverage, deferred). WBF watch: still internal (not in PUBLIC-SURFACE.md) though advancing fast (d1f0ebec/aac3737a WorldBelief fuse/query/step API, I-INIT-WBF) — keep deferring until public. Next Arm-E area 3/6: evidence algebra REFINE/REPROCESS.
- **2026-06-24 (iter 47, Arm-E area 3/6)** Evidence algebra REFINE/REPROCESS — found+fixed a REAL duplication: my iter-32 evidence-refinement.mdx fully re-documented REPROCESS EDGES, which the pre-existing nav-linked skills/reprocess-edges.mdx already owns (the grep-before-new-page rule missed it then). FIX: skills/reprocess-edges.mdx = canonical for REPROCESS (skill re-run model, when-to-use, examples); evidence-refinement.mdx REPROCESS section trimmed to syntax + cross-link to canonical; REFINE EDGE stays canonical in evidence-refinement (per-edge), now linked from reprocess-edges. Bidirectional cross-links added. WBF watch: still internal (PUBLIC-SURFACE count 0). Next Arm-E area 4/6: live views (concept vs statement vs wire).
- **2026-06-24 (iter 48, Arm-E area 4/6)** Live views: 3-layer split AUDITED coherent (concept docs/live-queries.mdx / statement worldcypher/statements/live-views.mdx / wire jsonrpc-v1 §Live-views — clean cross-links, no cross-layer redef). Found+removed a STALE ORPHAN: docs/reference/extensions/triggered-write-back-views.mdx (generated file with NO catalog source). Arm-G FACT-CHECK (vs engine) prevented a wrong rename: engine's db.extensions() literally returns 'Reactive write-back views' (db_extensions_procs.rs:36; lib.rs:8822 'write-back views are reactive') — so 'Reactive' is the CORRECT current term, reactive-write-back-views.mdx is right (sourced, --check green), and 'triggered' was the orphan. Fixed the 1 inbound link (live-surface.mdx → reactive). Generator --check green after removal. Next Arm-E area 5/6: lake:// virtual labels.
- **2026-06-24 (iter 49, Arm-E area 5/6)** lake:// virtual labels: only TWO pages spell out the lake://<mount>/<table>/{var}.../<glob>.parquet URI template — create-node-label.mdx (DDL+URI canonical, iter 34) and guides/lakehouse-fast-path.mdx (how-to). Established create-node-label as canonical; added a callout in the guide's Step 2 deferring to it for the canonical DDL+URI grammar (guide keeps its worked-pipeline value-add: mount config, manifest atomic-commit, registration). Other lake:// mentions (world-store/perception-lake/worldgraph/virtual-computed-columns) only reference in passing — no redef. WBF watch: still internal (count 0; EvidentialField C10 added this cycle). Next Arm-E area 6/6 (LAST): clock domains/temporal.
- **2026-06-24 (iter 50, Arm-E area 6/6 — FINAL)** Clock domains/temporal: AUDITED coherent. temporal.mdx = canonical concept home (clock domains §111, AS OF §157, deterministic replay, gate); worldcypher/temporal.mdx = query-syntax reference (AS OF seq syntax + temporal.* procedures velocity/decay/trajectory) — no concept redef, AS-OF-timestamp-rejected fact consistent in both, already cross-links up to /temporal. Added an inline up-link at the AS-OF-seq definition. **ARM-E SSoT PASS COMPLETE (6/6).**

## 🎯 MISSION STEADY-STATE — 2026-06-24 (iter 50)
All seven arms resolved: gap matrix DRAINED (8 clusters), Arm-G vocab sweep DONE, Arm-E SSoT pass DONE (6/6 areas). Entering IDLE-POLL (~1h cadence): each wake = standing sweeps a-d + WBF public-surface watch only; act only on new public surface / feat / inbox / contradiction.
- **2026-06-24 (iter 52, idle-poll — federation action)** Handled NEW fed ask AF-DOC-2026-06-24-001 (arcflow-core proactive-fix): catalog §Relationship Skills (EXT-0002) had phantom CREATE SKILL syntax (FROM EMBEDDING THRESHOLD... ALLOWED ON Document — unparseable). Re-vendored: fixed docs/reference/data/arcflow-extensions-catalog.md → 'CREATE SKILL similar_docs FROM EMBEDDING embedding THRESHOLD 0.8 ALLOWED ON [Document] TIER NEURAL', regenerated relationship-skills.mdx (--check green), mirrored resolution to docs federation inbox. Other sweeps clean; WBF still internal (count 0). Back to idle-poll.
